...
fn test_merkleize() {
    ...
    let (mut test_comm, _root) = ligero_commit(&random_ml_coeffs, &aux);
...
fn test_eval_outer() {
    ...
    let (test_comm, _root) = ligero_commit(&random_ml_coeffs, &aux);
...
fn test_open_column() {
    ...
    let test_comm = {
        ...
        let (mut tmp, _root) = ligero_commit(&random_ml_coeffs, &aux);
        merkleize(&mut tmp);
        tmp
    };
