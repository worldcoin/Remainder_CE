...
pub fn ligero_commit<F: Halo2FFTFriendlyField>(
...
pub fn ligero_eval_prove<F: Halo2FFTFriendlyField>(
...
///   `challenge_coord`.
pub fn ligero_verify<F: Halo2FFTFriendlyField>(
    commit_root: &F,
    aux: &LigeroAuxInfo<F>,
...
mod tests {
    ...
    use super::{ligero_commit, ligero_eval_prove, ligero_verify};

    /// This tests what the Remainder interface should be doing, i.e. using the
    /// [ligero_commit()], [ligero_eval_prove()], and
    /// [ligero_verify()] functions.
    ///
    /// In particular, we
    ...
    fn test_remainder_flow() {
        ...
        let (comm, root) = ligero_commit(&ml_coeffs, &aux);

        // Add commitment to transcript
        transcript_writer.append("root", root.root);

        let _prove_result = ligero_eval_prove(
            &ml_coeffs,
            &challenge_coord,
