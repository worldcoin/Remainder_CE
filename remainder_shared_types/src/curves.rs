...
/// + The trait in the halo2curves library that is closest to what is wanted is
///   `CurveExt`.  However, the field trait they use for the base and scalar
///   fields, viz. `WithSmallOrderMulGroup<3>` is not appropriate, however, as
///   it restricts to finite fields for which p - 1 is divisible by 3.  This is
///   an arbitrarily restriction from our POV (though it is satisfied by Bn254).
///   (Further, we found the halo2curves traits are very difficult to parse).
/// + The `AffineCurve` trait from `ark-ec` is precisely as specific as required
///   and are beautifully written, but we'd need to implement the arkworks field
///   traits for the fields we use from halo2.
use std::{
    fmt,
    ops::{Add, AddAssign, Mul, MulAssign, Neg, Sub, SubAssign},
};
use subtle::{Choice, ConditionallySelectable};
// Implementation note: do not confuse the following with the very similarly
// named remainder_shared_types::halo2curves::Group
    ...
    + Zeroizable
{
    ...
    /// Return the affine coordinates of the point, if it is not at the identity
    ...
    /// TODO!(ryancao): Should we also have a self type representing the
    /// bitwidth?
    fn to_bytes_compressed(&self) -> Vec<u8>;

    /// Returns the unique curve element represented by the uncompressed
    /// bytestring.
    fn from_bytes_uncompressed(bytes: &[u8]) -> Self;

    /// Returns the unique curve element represented by the compressed
    /// bytestring.
    ...
    /// An optimized version of scalar multiplication when the scalar element
...
impl Zeroizable for Fq {
    fn zeroize(&mut self) {
        *self = Fq::ZERO;
    }
}

impl Zeroizable for Bn256 {
    fn zeroize(&mut self) {
        *self = Bn256::zero();
    }
}
...
impl PrimeOrderCurve for Bn256 {
    ...
    fn projective_coordinates(&self) -> (Self::Base, Self::Base, Self::Base) {
        if let Some((x, y)) = self.affine_coordinates() {
            ...
            (Self::identity().x, Self::identity().y, Self::identity().z)
        }
    }
    ...
    /// * The next 32 `u8` bytes represent the x-coordinate of the point in
    ///   little endian.
    /// * The next 32 `u8` bytes represent the y-coordinate of the point in
    ///   little endian.
    fn to_bytes_uncompressed(&self) -> Vec<u8> {
        // --- First get the affine coordinates. If `None`, we have a point at
        // infinity. ---
    ...
    fn to_bytes_compressed(&self) -> Vec<u8> {
        ...
        if let Some((x, y)) = affine_coords {
            let x_bytes = x.to_bytes();
            // 0 when y < q/2, 1 when y > q/2, where q is the prime order of
            // base field this is because when y < q/2 and y is a square root,
            // this means y is even. and if y > q/2 and y is a square root, this
            // means y is odd. this is exactly what we are computing using & 1.
    ...
    /// will return the elliptic curve point corresponding to an array of bytes
    /// that represent an uncompressed point. we represent it as a a normalized
    /// projective curve point (ie, the x and y coordinates are directly the
    /// affine coordinates) so the z coordinate is always 1.
    fn from_bytes_uncompressed(bytes: &[u8]) -> Self {
    ...
    /// will return the elliptic curve point corresponding to an array of bytes
    /// that represent a compressed point. we represent it as a a normalized
    /// projective curve point (ie, the x and y coordinates are directly the
    /// affine coordinates) so the z coordinate is always 1.
    fn from_bytes_compressed(bytes: &[u8]) -> Self {
        // first check if it is a point at infinity
    ...
    /// Simple double-and-add method for scalar multiplication, optimized for
    /// when the scalar is representable as an unsigned integer (u8, u16, u32,
    /// u64, or u128).
    fn scalar_mult_unsigned_integer<T: Unsigned + Zero + ToBytes>(&self, scalar: &T) -> Self {
        let scalar_repr = scalar.to_be_bytes();
        let bits = scalar_repr
            .as_ref()
            .iter()
        ...
        for bit in bits {
            ...
            // The conditional_select is a constant-time selector using the XOR
            // method.
            acc = Self::conditional_select(&acc, &(acc + self), Choice::from(bit as u8));
        }
        acc
    }
}

/// wrapper needed in order to implement RngCore for the Sha256 digest. we need
/// this to implement RngCore so that it can be used to generate random group
/// elements by calling `PrimeOrderCurve::random`
