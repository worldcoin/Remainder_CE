...
use super::{
    data_pipeline::dt2zkdt::{
        circuitize_auxiliaries, load_raw_samples, load_raw_trees_model, CircuitizedAuxiliaries,
        CircuitizedSamples, CircuitizedTrees, RawSamples, RawTreesModel, Samples, TreesModel,
    },
    structs::{
        BinDecomp16Bit, BinDecomp4Bit, BinDecomp8Bit, DecisionNode, InputAttribute, LeafNode,
    },
};
use remainder::{layer::LayerId, mle::dense::DenseMle, prover::input_layer};
...
pub fn convert_zkdt_circuit_data_multi_tree_into_mles<F: FieldExt>(
    ...
    let input_samples_mle_vec = input_data_vec[0]
        .clone()
        .into_iter()
        .map(|input| {
    ...
    let permuted_input_samples_mle_vec_vec = permuted_input_data_vec
        .into_iter()
    ...
    let decision_node_paths_mle_vec_vec: Vec<Vec<DenseMle<F, DecisionNode<F>>>> =
        decision_node_paths_vec
            .into_iter()
            .map(|decision_node_paths| {
                decision_node_paths
                    .iter()
                    .map(|path| {
                        DenseMle::new_from_iter(path.clone().into_iter(), LayerId::Input(0), None)
                    })
                    .collect()
            })
            .collect_vec();

    let leaf_node_paths_mle_vec_vec = leaf_node_paths_vec
        .into_iter()
        .map(|leaf_node_paths| {
            leaf_node_paths
                .into_iter()
    ...
    let binary_decomp_diffs_mle_vec_vec = binary_decomp_diffs_vec
        .into_iter()
    ...
    let multiplicities_bin_decomp_mle_input = multiplicities_bin_decomp_input_vec[0]
        ...
        .map(|datum| {
            DenseMle::new_from_iter(
                datum.clone().into_iter().map(BinDecomp8Bit::from),
                LayerId::Input(0),
                None,
            )
        })
