...
use remainder::expression::{Expression, ExpressionStandard};
use remainder::layer::batched::BatchedLayer;
use remainder::layer::{LayerBuilder, LayerId};
use remainder::mle::dense::{DenseMle, DenseMleRef, Tuple2, TupleTree};
use remainder::mle::MleRef;
use remainder::mle::{zero::ZeroMleRef, Mle, MleIndex};
use remainder::sumcheck::compute_sumcheck_message;
...
impl<F: FieldExt> LayerBuilder<F> for DumbProduct<F> {
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        let prod_bt = (0..self.mles[0].mle_ref().current_mle.len())
            .map(|idx| {
                self.mles
                    .iter()
                    .fold(F::one(), |acc, mle| mle.mle_ref().current_mle[idx] * acc)
            })
            .collect_vec();
...
impl<F: FieldExt> DumbProduct<F> {
    pub fn new(mles: Vec<DenseMle<F, F>>) -> Self {
        Self { mles }
    }
}
...
impl<F: FieldExt> DumbBuilder<F> {
    /// creates new difference mle
    pub fn new(mle_1: DenseMle<F, F>) -> Self {
        Self { mle_1 }
    }
}
...
impl<F: FieldExt> LayerBuilder<F> for AttributeConsistencyBuilder<F> {
    ...
    fn build_expression(&self) -> ExpressionStandard<F> {
        ExpressionStandard::Mle(
            self.mle_input
                .attr_id(Some(log2(self.tree_height) as usize)),
        ...
        DenseMle::new_from_iter(
            self.mle_input
                ...
                .map(
                    |(
                        InputAttribute {
                            attr_id: input_attr_ids,
                            ..
                        },
                        DecisionNode {
                            attr_id: path_attr_ids,
                            ..
                        },
...
impl<F: FieldExt> LayerBuilder<F> for AttributeConsistencyBuilderZeroRef<F> {
    ...
    fn build_expression(&self) -> ExpressionStandard<F> {
        ExpressionStandard::Mle(
            self.mle_input
                .attr_id(Some(log2(self.tree_height - 1) as usize)),
        ) - ExpressionStandard::Mle(self.mle_path.attr_id())
    }
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        ...
        DenseMle::new_from_iter(
            self.mle_first
                ...
                .current_mle
                .into_iter()
                .zip(self.mle_second.clone().current_mle)
                .map(|(a, b)| a * b),
            id,
            prefix_bits,
        )
...
impl<F: FieldExt> SplitProductBuilderTupleTree<F> {
    /// create new halfed multiplied mle
    pub fn new(mle_first: DenseMleRef<F>, mle_second: DenseMleRef<F>) -> Self {
        Self {
            mle_first,
            mle_second,
        }
...
impl<F: FieldExt> LayerBuilder<F> for SplitProductBuilder<F> {
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        ...
        let split_mle = self.mle.split(F::one());
        DenseMle::new_from_iter(
            split_mle.into_iter().map(|Tuple2((a, b))| a * b),
            id,
            prefix_bits,
        )
    }
...
impl<F: FieldExt> LayerBuilder<F> for IdentityBuilder<F> {
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        DenseMle::new_from_iter(self.mle_1.into_iter(), id, prefix_bits)
    }
}
...
impl<F: FieldExt> LayerBuilder<F> for ConcatBuilder<F> {
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        ...
        (0..range_size).for_each(|idx| {
            ...
            let first_mle_val = first_mle_ref.current_mle.get(index_1).unwrap_or(zero);
            let second_mle_val = second_mle_ref.current_mle.get(index_2).unwrap_or(zero);
...
impl<F: FieldExt> LayerBuilder<F> for FSRMinusXBuilder<F> {
    ...
    fn build_expression(&self) -> ExpressionStandard<F> {
        ExpressionStandard::Mle(self.r_mle.mle_ref())
            - (ExpressionStandard::Mle(self.packed_x.mle_ref()))
    }

    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        let r = self.r_mle.mle_ref().current_mle[0];
        DenseMle::new_from_iter(
            self.packed_x.clone().into_iter().map(|x| r - x),
            id,
            prefix_bits,
        )
    }
...
impl<F: FieldExt> FSRMinusXBuilder<F> {
    /// create new leaf node packed
    pub fn new(packed_x: DenseMle<F, F>, r_mle: DenseMle<F, F>) -> Self {
        Self { packed_x, r_mle }
    }
}
...
impl<F: FieldExt> LayerBuilder<F> for RMinusXBuilder<F> {
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        DenseMle::new_from_iter(
            self.packed_x.clone().into_iter().map(|x| self.r - x),
            id,
            prefix_bits,
        )
    }
...
impl<F: FieldExt> RMinusXBuilder<F> {
    /// create new leaf node packed
    pub fn new(packed_x: DenseMle<F, F>, r: F) -> Self {
        Self { packed_x, r }
    }
}
...
impl<F: FieldExt> LayerBuilder<F> for SquaringBuilder<F> {
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        DenseMle::new_from_iter(self.mle.clone().into_iter().map(|x| x * x), id, prefix_bits)
    }
}
...
impl<F: FieldExt> LayerBuilder<F> for BitExponentiationBuilder<F> {
    ...
    fn build_expression(&self) -> ExpressionStandard<F> {
        ...
        ExpressionStandard::Sum(
            Box::new(ExpressionStandard::products(vec![
                self.r_minus_x_power.mle_ref(),
                b_ij.clone(),
            ])),
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        ...
        let b_ij = self.bin_decomp.mle_bit_refs()[15 - self.bit_index].clone();
        DenseMle::new_from_iter(
            self.r_minus_x_power
                .clone()
                .into_iter()
                .zip(b_ij.bookkeeping_table.into_iter())
                .map(|(r_minus_x_power, b_ij_iter)| {
                    (b_ij_iter * r_minus_x_power + (F::one() - b_ij_iter))
                }),
            id,
            prefix_bits,
        )
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        DenseMle::new_from_iter(
            self.prev_prod
                ...
                .map(|(prev_prod, multiplier)| prev_prod * multiplier),
            id,
            prefix_bits,
        )
    }
...
impl<F: FieldExt> ProductBuilder<F> {
    /// create new leaf node packed
    pub fn new(multiplier: DenseMle<F, F>, prev_prod: DenseMle<F, F>) -> Self {
        Self {
            multiplier,
            prev_prod,
        }
...
impl<F: FieldExt> LayerBuilder<F> for LeafPackingBuilder<F> {
    ...
    fn build_expression(&self) -> ExpressionStandard<F> {
        ExpressionStandard::Constant(self.r)
            - (ExpressionStandard::Mle(self.mle.node_id())
                + ExpressionStandard::Scaled(
                    Box::new(ExpressionStandard::Mle(self.mle.node_val())),
                    self.r_packing,
                ))
    }
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        DenseMle::new_from_iter(
            self.mle.into_iter().map(|LeafNode { node_id, node_val }| {
...
impl<F: FieldExt> LeafPackingBuilder<F> {
    /// create new leaf node packed
    pub(crate) fn new(mle: DenseMle<F, LeafNode<F>>, r: F, r_packing: F) -> Self {
        Self { mle, r, r_packing }
    }
}
...
impl<F: FieldExt> LayerBuilder<F> for FSLeafPackingBuilder<F> {
    ...
    fn build_expression(&self) -> ExpressionStandard<F> {
        ExpressionStandard::Mle(self.r_mle.mle_ref())
            - (ExpressionStandard::Mle(self.mle.node_id())
                + ExpressionStandard::products(vec![
                    self.mle.node_val(),
                    self.r_packing_mle.mle_ref(),
                ]))
    }

    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        let r = self.r_mle.mle_ref().current_mle[0];
        let r_packing = self.r_packing_mle.mle_ref().current_mle[0];

        DenseMle::new_from_iter(
            self.mle
                .into_iter()
                .map(|LeafNode { node_id, node_val }| r - (node_id + r_packing * node_val)),
            id,
            prefix_bits,
        )
    }
...
impl<F: FieldExt> LayerBuilder<F> for DecisionPackingBuilder<F> {
    ...
    fn build_expression(&self) -> ExpressionStandard<F> {
        ExpressionStandard::Constant(self.r)
            - (ExpressionStandard::Mle(self.mle.node_id())
                + ExpressionStandard::Scaled(
                    Box::new(ExpressionStandard::Mle(self.mle.attr_id())),
                    self.r_packings.0,
                )
                + ExpressionStandard::Scaled(
                    Box::new(ExpressionStandard::Mle(self.mle.threshold())),
                    self.r_packings.1,
                ))
    }
...
impl<F: FieldExt> DecisionPackingBuilder<F> {
    ...
    pub(crate) fn new(mle: DenseMle<F, DecisionNode<F>>, r: F, r_packings: (F, F)) -> Self {
        Self { mle, r, r_packings }
    }
}
...
impl<F: FieldExt> LayerBuilder<F> for FSDecisionPackingBuilder<F> {
    ...
    fn build_expression(&self) -> ExpressionStandard<F> {
        ExpressionStandard::Mle(self.r_mle.mle_ref())
            - (ExpressionStandard::Mle(self.mle.node_id())
                ...
                + ExpressionStandard::products(vec![
                    self.mle.threshold(),
                    self.r_packing_another_mle.mle_ref(),
                ]))
    }
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        let r = self.r_mle.mle_ref().current_mle[0];
        let r_packing = self.r_packing_mle.mle_ref().current_mle[0];
        let r_packing_another = self.r_packing_another_mle.mle_ref().current_mle[0];
...
impl<F: FieldExt> LayerBuilder<F> for FSInputPackingBuilder<F> {
    ...
    fn build_expression(&self) -> ExpressionStandard<F> {
        let res = ExpressionStandard::Mle(self.r_mle.mle_ref())
            - (ExpressionStandard::Mle(self.mle.attr_id(None))
                + ExpressionStandard::products(vec![
                    self.mle.attr_val(None),
                    self.r_packing_mle.mle_ref(),
                ]));
        // dbg!(&res);
        res
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        let r = self.r_mle.mle_ref().current_mle[0];
        let r_packing = self.r_packing_mle.mle_ref().current_mle[0];
        let res = DenseMle::new_from_iter(
            self.mle
                .into_iter()
                .map(|InputAttribute { attr_id, attr_val }| r - (attr_id + r_packing * attr_val)),
            id,
            prefix_bits,
        );
        res
...
impl<F: FieldExt> LayerBuilder<F> for InputPackingBuilder<F> {
    ...
    fn build_expression(&self) -> ExpressionStandard<F> {
        ExpressionStandard::Constant(self.r)
            - (ExpressionStandard::Mle(self.mle.attr_id(None))
                + ExpressionStandard::Scaled(
                    Box::new(ExpressionStandard::Mle(self.mle.attr_val(None))),
                    self.r_packing,
                ))
    }
    ...
    fn next_layer(&self, id: LayerId, prefix_bits: Option<Vec<MleIndex<F>>>) -> Self::Successor {
        DenseMle::new_from_iter(
            self.mle
                .into_iter()
                .map(|InputAttribute { attr_id, attr_val }| {
...
impl<F: FieldExt> InputPackingBuilder<F> {
    /// create new decision node packed
    pub(crate) fn new(mle: DenseMle<F, InputAttribute<F>>, r: F, r_packing: F) -> Self {
        Self { mle, r, r_packing }
    }
}
...
mod tests {
    ...
    use crate::{
        ...
        data_pipeline::dummy_data_generator::{
            generate_dummy_mles, generate_dummy_mles_batch, BatchedDummyMles, DummyMles,
            NUM_DUMMY_INPUTS, TREE_HEIGHT,
        },
        input_data_to_circuit_adapter::BatchedZKDTCircuitMles,
    };
    use ark_std::log2;
    use remainder::mle::{dense::DenseMle, MleRef};
    use remainder_shared_types::{halo2curves::FieldExt, Fr};
    ...
    fn test_input_packing_builder() {
        ...
        let next_layer_should_be = dummy_input_data_mle
            .attr_id(None)
            .bookkeeping_table
            .iter()
        ...
        assert_eq!(next_layer.mle_ref().current_mle, next_layer_should_be);
        println!("layer mle: {:?}", next_layer.mle_ref().current_mle);
        ...
        assert_eq!(
            next_layer_should_be,
            vec![Fr::from(16162).neg(), Fr::from(11898).neg()]
        );
    }
    ...
    fn test_decision_node_packing_builder() {
        ...
        let next_layer = input_packing_builder.next_layer(LayerId::Layer(0), None);
        let next_layer_should_be = dummy_decision_node_paths_mle
            .node_id()
            .bookkeeping_table
            .iter()
            .zip(
    ...
    fn test_leaf_node_packing_builder() {
        ...
        let next_layer = input_packing_builder.next_layer(LayerId::Layer(0), None);
        let next_layer_should_be = dummy_leaf_node_paths_mle
            .node_id()
            .bookkeeping_table
            .iter()
            .zip(
        ...
        assert_eq!(
            next_layer_should_be,
            vec![Fr::from(3) - (Fr::from(2) + Fr::from(5) * Fr::from(17299145535799709783_u64))]
        );
    }
    ...
    fn test_expo_builder() {
        ...
        // characteristic poly w packing: [3 - (0 + 5 * 0 + 4 * 1206 )], which is [-4821]
        assert_eq!(
            decision_packed.mle_ref().current_mle,
        ...
        assert_eq!(
            leaf_packed.mle_ref().current_mle,
            vec![
        ...
        for i in 1..16 {
            // (r-x)^2
            let r_minus_x_square_builder = SquaringBuilder { mle: r_minus_x };
    ...
    fn test_multiset_builder_dummy() {
        ...
        assert_eq!(
            exponentiated_nodes.mle_ref().current_mle,
            vec![
                Fr::from(6 + 8342).pow(&[4_u64, 0, 0, 0])
                    * (Fr::from(3)
                        + (Fr::from(1) + Fr::from(6) * Fr::from(4845552296702772050_u64)))
                    .pow(&[1_u64, 0, 0, 0])
                    * (Fr::from(3)
                        + (Fr::from(2) + Fr::from(6) * Fr::from(3424474836643299239_u64)))
                    .pow(&[3_u64, 0, 0, 0])
            ]
        );
        ...
        let mut prev_prod_x_path_packed: DenseMle<Fr, Fr> =
            DenseMle::new_from_raw([Fr::from(1); TREE_HEIGHT].to_vec(), LayerId::Input(0), None);
        ...
        for i in 0..NUM_DUMMY_INPUTS {
            ...
            let decision_path_packed =
                decision_path_packing_builder.next_layer(LayerId::Layer(0), None);
            assert_eq!(
                decision_path_packed.mle_ref().current_mle,
                vec![Fr::from(8342).neg()]
            );
            ...
            if i == 1 {
                assert_eq!(
                    leaf_path_packed.mle_ref().current_mle,
                    vec![
                        (Fr::from(3)
                            - (Fr::from(1) + Fr::from(6) * Fr::from(4845552296702772050_u64)))
                    ]
            ...
            let x_path_packed =
                decision_leaf_path_concat_builder.next_layer(LayerId::Layer(0), None);

            if i == 1 {
                assert_eq!(
                    x_path_packed.mle_ref().current_mle,
                    vec![
                        Fr::from(8342).neg(),
                        (Fr::from(3)
                            - (Fr::from(1) + Fr::from(6) * Fr::from(4845552296702772050_u64)))
            ...
            if i == 1 {
                assert_eq!(
                    curr_x_path_packed.mle_ref().current_mle,
                    vec![
                        Fr::from(6 + 8342),
                        (Fr::from(3)
                            + (Fr::from(1) + Fr::from(6) * Fr::from(4845552296702772050_u64)))
        ...
        assert_eq!(
            path_exponentiated.mle_ref().current_mle,
            vec![
                Fr::from(6 + 8342).pow(&[4_u64, 0, 0, 0]),
                (Fr::from(3) + (Fr::from(1) + Fr::from(6) * Fr::from(4845552296702772050_u64)))
                    .pow(&[1_u64, 0, 0, 0])
    ...
    fn test_attribute_consistency_builder_dummy() {
        ...
        for i in 0..NUM_DUMMY_INPUTS {
            ...
            assert_eq!(difference_mle.mle_ref().current_mle, zero_vec)
        }
    }
    ...
    fn test_permutation_builder_catboost() {
        ...
        for i in 0..num_dummy_inputs {
            ...
            let permuted_input_packed =
                permuted_input_packing_builder.next_layer(LayerId::Layer(0), None);
            ...
            for _ in 0..log2(input_len) {
                let prod_builder = SplitProductBuilder { mle: input_prod };
                let _ = prod_builder.build_expression();
            ...
            assert_eq!(
                input_prod.mle_ref().current_mle,
                permuted_input_prod.mle_ref().current_mle
            )
        }
    }
    ...
    fn test_multiset_builder_catboost() {
        ...
        for i in 1..16 {
            ...
            let r_minus_x_square_decision =
                r_minus_x_square_builder_decision.next_layer(LayerId::Layer(i + 1), None);

            let r_minus_x_square_builder_leaf = SquaringBuilder {
                mle: r_minus_x_leaf,
            };
            let _ = r_minus_x_square_builder_leaf.build_expression();
            let r_minus_x_square_leaf =
                r_minus_x_square_builder_leaf.next_layer(LayerId::Layer(i + 1), None);
            ...
            let curr_prod_decision =
                curr_prod_builder_decision.next_layer(LayerId::Layer(i + 2), None);
        ...
        let mut prev_prod_x_path_packed: DenseMle<Fr, Fr> =
            DenseMle::new_from_raw(one_vec.to_vec(), LayerId::Input(0), None);
