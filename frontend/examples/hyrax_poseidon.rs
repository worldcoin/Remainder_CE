...
fn build_circuit(num_leafs: usize) -> Circuit<Fr> {
    assert!(num_leafs <= T - 1);
    let mut builder = CircuitBuilder::<Fr>::new();

    let param_layer = builder.add_input_layer("Parameters", LayerVisibility::Public);
    let content_layer =
    ...
    let mds_matrix = builder.add_input_shred("MDS Matrix", 2 * NUM_VARS_T, &param_layer);
    ...
    let root_hash = merkle_path(&mut builder, leafs, vec![], &round_consts, &mds_matrix);
...
fn gen_poseidon_test(inputs: Vec<Fr>) -> (
    Vec<MultilinearExtension<Fr>>, // constants,
    MultilinearExtension<Fr>, // mds matrix,
    Vec<MultilinearExtension<Fr>>, // inputs
    MultilinearExtension<Fr>, // outputs
) {
    assert_eq!(inputs.len(), NUM_LEAFS);
    let pad_len = T.next_power_of_two();
    // specs
    ...
    let leafs_data = inputs.into_iter().map(|input| MultilinearExtension::new(vec![input])).collect();
    let expected_hash_data = MultilinearExtension::new(vec![poseidon.state()[0]]);
    (constants, mds_matrix, leafs_data, expected_hash_data)
}
...
fn main() {
    ...
    let base_circuit = build_circuit(NUM_LEAFS);
    let mut prover_circuit = base_circuit.clone();
    let verifier_circuit = base_circuit.clone();

    // Generate circuit inputs.
    let (constants, mds_matrix, leafs_data, expected_hash_data) = gen_poseidon_test(vec![Fr::from(2), Fr::from(3)]);
    ...
    prover_circuit.set_input("Expected root hash", expected_hash_data); // This is public!
