...
fn full_poseidon(
    ...
    mut input_by_col: Vec<NodeRef<Fr>>,
    round_consts: &Vec<Vec<Fr>>,
    mds_matrix: &NodeRef<Fr>,
) -> Vec<NodeRef<Fr>> {
    // Prepend IV
    ...
    let zero_mle = builder.add_sector(AbstractExpression::binary_tree_selector(vec![
        zero;
        PATH_LEN
    ]));
    ...
    for i in 0..R_F / 2 {
        ...
        // state = state^5
        let mut state = builder.add_sector(AbstractExpression::binary_tree_selector(
            input_by_col.clone(),
        ));
...
fn gen_poseidon_spec() -> (
    ...
    let constants = constants
        .into_iter()
        .map(|round_consts| round_consts.to_vec())
        .collect();
...
fn gen_poseidon_test(
    ...
    for _ in 0..num_poseidons {
        ...
        init_state
            .into_iter()
            .enumerate()
            .for_each(|(i, v)| init_states[i].push(v));
        final_state
            .into_iter()
            .enumerate()
            .for_each(|(i, v)| final_states[i].push(v));
    }
    let init_states = init_states
        .into_iter()
        .map(|init_state| MultilinearExtension::new(init_state))
        .collect();
