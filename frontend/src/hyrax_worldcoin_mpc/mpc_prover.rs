...
use hyrax::{
    gkr::{
        input_layer::{commit_to_input_values, HyraxProverInputCommitment},
...
pub fn print_features_status() {
    ...
    println!("=== FEATURES ===");
    println!(
        "Parallel feature for prover: {}",
        STATUS_STR[remainder::utils::is_parallel_feature_on() as usize]
    );
    println!(
        "Parallel feature for hyrax: {}",
        STATUS_STR[hyrax::utils::is_parallel_feature_on() as usize]
    );
    println!(
    ...
    println!(
        "Claim aggregation constant column optimization for prover: {}",
