...
pub fn load_image_commitment(version: u8, mask: bool, left_eye: bool) -> SerializedImageCommitment {
    ...
    let image_fn =
        format!("iriscode_pcp_example/{eye}_normalized_{image_or_mask}{sizing_suffix}.bin");
    let image = read_bytes_from_file(&image_fn);
    let commitment_fn = format!(
        "iriscode_pcp_example/{eye}_normalized_{image_or_mask}_commitment{sizing_suffix}.bin"
    );
    let commitment_bytes = read_bytes_from_file(&commitment_fn);
    let blinding_factors_fn = format!(
        "iriscode_pcp_example/{eye}_normalized_{image_or_mask}_blinding_factors{sizing_suffix}.bin"
    );
