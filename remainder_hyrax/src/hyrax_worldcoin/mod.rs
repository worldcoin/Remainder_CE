...
///
/// All the input layers in this circuit are public input layers.
///
/// See [CircuitData] for a detailed description of each generic and argument with all public input layers.
pub fn build_hyrax_circuit_public_input_layer<
    C: PrimeOrderCurve,
...
>(
    ...
    move |ctx| {
        ...
        let input_layer_data =
            HyraxInputLayerData::new(input_layer.id(), input_data_shreds, None, None);
        ...
        (
            ComponentSet::<NodeEnum<C::Scalar>>::new_raw(all_nodes),
...
/// + the length of the MLE `to_reroute`.
///
/// The input layers of this circuit are either public, if it is okay to send this information in the clear,
/// or Hyrax PCS input layers, which means there is a blinded commitment to the input data. We do not
/// include the orb commitment here.
///
/// See [CircuitData] for a detailed description of each generic and argument with all public input layers.
pub fn build_hyrax_circuit_hyrax_input_layer<
    C: PrimeOrderCurve,
    const MATMULT_ROWS_NUM_VARS: usize,
    const MATMULT_COLS_NUM_VARS: usize,
    const MATMULT_INTERNAL_DIM_NUM_VARS: usize,
...
>(
    ...
    move |ctx| {
        ...
        println!("{:?} = Public Input layer", public_input_layer.id());
        let hyrax_input_layer_for_reroute =
            InputLayerNode::new(ctx, None, InputLayerType::HyraxInputLayer);
        println!(
            "{:?} = Hyrax Input layer for source of rerouting",
            hyrax_input_layer_for_reroute.id()
        );
        let hyrax_input_layer_for_digits_and_multiplicities =
            InputLayerNode::new(ctx, None, InputLayerType::HyraxInputLayer);
        println!(
            "{:?} = Hyrax Input layer for digits and multiplicities",
        ...
        let public_input_data_shreds = vec![
            rh_matmult_multiplicand_data,
            sign_bits_data,
            to_sub_from_matmult_data,
            lookup_table_values_data,
        ];
        let hyrax_input_data_shreds_to_reroute = vec![to_reroute_data];
        let mut hyrax_input_data_shreds_multiplicities_digits = vec![digit_multiplicities_data];
        hyrax_input_data_shreds_multiplicities_digits.extend(digits_input_shreds_data);
        let public_input_layer_data = HyraxInputLayerData::new(
            public_input_layer.id(),
            public_input_data_shreds,
            None,
            None,
        );
        ...
        let hyrax_input_layer_data_for_digits_and_multiplicities = HyraxInputLayerData::new(
            hyrax_input_layer_for_digits_and_multiplicities.id(),
            hyrax_input_data_shreds_multiplicities_digits,
            None,
            None,
        );
        ...
        (
            ComponentSet::<NodeEnum<C::Scalar>>::new_raw(all_nodes),
            vec![
                public_input_layer_data,
                hyrax_input_layer_data_for_rerouting,
                hyrax_input_layer_data_for_digits_and_multiplicities,
            ],
        )
    }
}
