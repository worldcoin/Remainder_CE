...
pub(crate) fn verify_iriscode(
    ...
    if *expected_aux_mle != aux_mle_in_proof {
        return Err(UpgradeError::IncorrectKernelValuesOrThresholds(version, is_left_eye, is_mask));
    }
    ...
    let code: Vec<bool> = code_mle
        ...
        .map(|b|
            match b {
                &Fr::ONE => Ok(true),
                &Fr::ZERO => Ok(false),
                _ => Err(UpgradeError::NonBinaryIrisMaskCode(version, is_left_eye, is_mask)),
            }
    ...
    for &b in &code[code_len..] {
        if b {
            return Err(UpgradeError::NonZeroPaddingBits(version, is_left_eye, is_mask));
        }
    }
