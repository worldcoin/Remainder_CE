...
impl<C: PrimeOrderCurve> ProofOfDotProduct<C> {
    /// Generate a proof that the dot product of `x_vector` and `a_vector` is
    /// `dot_prod_val_y`. As a part of weak FS, the calling context is
    /// responsible for adding commitments to x_vector and dot_prod_val_y to the
    ...
    pub fn prove(
        ...
        let mut r_delta = C::Scalar::random(&mut rng);
        // blinding factor to commit to the dot product of d and a
        let mut r_beta = C::Scalar::random(&mut rng);
    ...
    /// the function that uses the evaluation proof (that is stored in &self)
    /// and the inputs to the "circuit" which are com_x (commitment to x), com_y
    /// (commitment to <x, a>), and a_vector (which is the public vector a).
    /// Panics iff the proof is invalid.
    ///
    /// As a part of weak FS, calling context is responsible for adding
    /// commitments to x_vector and dot_prod_val_y to the transcript (if this is
    /// necessary).
    pub fn verify(
        // evaluation proof
        &self,
