...
impl<F: FieldExt> MatMultNode<F> {
    ...
    pub fn new(
        ...
        let matrix_a = Matrix::new(matrix_a_data, num_rows_cols_a.0, num_rows_cols_a.1, None);

        let matrix_b_data =
            MultilinearExtension::new(matrix_node_b.get_data().get_evals_vector().clone());
        let matrix_b = Matrix::new(matrix_b_data, num_rows_cols_b.0, num_rows_cols_b.1, None);
