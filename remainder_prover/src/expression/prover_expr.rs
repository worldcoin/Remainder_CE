...
impl<F: FieldExt> ExpressionType<F> for ProverExpression {
    type Container = DenseMleRef<F>; /// this also needs to change, perhaps duplicate a struct, rename, and then change
...
/// the leaf nodes of the expression tree are DenseMleRefs
#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct ProverExpressionMleVec;
impl<F: FieldExt> ExpressionType<F> for ProverExpressionMleVec {
...
impl<F: FieldExt> Expression<F, ProverExpressionMleVec> {
    ...
    // need to write this helper function,
    ...
    pub fn increment_mle_vec_indices(&mut self, offset: usize) {
        ...
        // use traverse_mut
        let mut increment_closure = for<'a, 'b> |expr: &'a mut ExpressionNode<F, ProverExpressionMleVec>, _mle_vec: &'b mut Vec<DenseMleRef<F>>| -> Result<(), ()> {
            match expr {
    ...
    pub fn concat_expr(mut self, lhs: Expression<F, ProverExpressionMleVec>) -> Self {
    ...
    pub fn products(product_list: <ProverExpressionMleVec as ExpressionType<F>>::MleVec) -> Self {
    ...
    /// mutable, don't want to fix variable twice, which is ok, because it won't contain indexedbit(round_idx) any more after the mle being mutabled for the first time
    ...
    pub fn evaluate_expr(mut self, challenges: Vec<F>) -> Result<F, ExpressionError> {
        ...
        let mut observer_fn = |exp: &ExpressionNode<F, ProverExpressionMleVec>, mle_vec: &<ProverExpressionMleVec as ExpressionType<F>>::MleVec| -> Result<(), ExpressionError> {
            match exp {
    ...
    /// mutate, but ok, because only index when mle.indexed == true
...
impl<F: FieldExt> ExpressionNode<F, ProverExpressionMleVec> {
    /// mutable but ok
    /// transforms the expression to a verifier expression
    /// should only be called when the entire expression is fully bound
    /// traverses the expression and changes the DenseMleRef to F,
    ...
    pub fn fix_variable_node(
        ...
        round_index: usize,
        challenge: F,
        mle_vec: &mut <ProverExpressionMleVec as ExpressionType<F>>::MleVec
    ) {
        match self {
            ExpressionNode::Selector(index, a, b) => {
...
/// ----------------------------old stuff above, this also needs to change------------------------------------
/// immutable
/// change by getting the mle_ref by using the index
...
/// change by getting the mle_ref by using the index
/// describes the circuit given the expression (includes all the info of the data that the expression is instantiated with)
impl<F: std::fmt::Debug + FieldExt> Expression<F, ProverExpressionMleVec> {
    pub(crate) fn circuit_description_fmt<'a>(&'a self) -> impl std::fmt::Display + 'a {

        struct CircuitDesc<'a, F: FieldExt>(&'a ExpressionNode<F, ProverExpressionMleVec>, &'a <ProverExpressionMleVec as ExpressionType<F>>::MleVec);
...
impl<F: FieldExt> Sub for Expression<F, ProverExpressionMleVec> {
    ...
    fn sub(mut self, rhs: Expression<F, ProverExpressionMleVec>) -> Expression<F, ProverExpressionMleVec> {
        ...
        let sum_node = ExpressionNode::Sum(Box::new(lhs_node), Box::new(rhs_node.neg()));
        let sum_mle_vec = lhs_mle_vec.into_iter().chain(rhs_mle_vec.into_iter()).collect_vec();
