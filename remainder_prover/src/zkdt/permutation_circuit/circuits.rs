...
use crate::{
    ...
    zkdt::builders::FSInputPackingBuilder,
};
...
use super::super::{
    builders::{EqualityCheck, InputPackingBuilder, SplitProductBuilder},
    ...
    fn synthesize_and_commit(
        ...
        for _ in 0..log2(input_len) {
            let prod_builder = BatchedLayer::new(
                input_packed
                    .into_iter()
                    .map(|input_packed| SplitProductBuilder::new(input_packed))
                    .collect(),
            );
            let prod_permuted_builder = BatchedLayer::new(
                input_permuted_packed
                    .into_iter()
                    .map(|input_permuted_packed| SplitProductBuilder::new(input_permuted_packed))
                    .collect(),
            );
        ...
        let difference_builder = EqualityCheck::new_batched(input_packed, input_permuted_packed);
        ...
        let input_packing_builder = BatchedLayer::new(
            self.input_data_mle_vec
                ...
                .map(|input_data_mle| {
                    ...
                    InputPackingBuilder::new(input_data_mle, self.r, self.r_packing)
                })
                .collect_vec(),
        );
...
impl<F: FieldExt> GKRCircuit<F> for NonBatchedPermutationCircuit<F> {
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        ...
        let input_packing_builder: InputPackingBuilder<F> =
            InputPackingBuilder::new(self.input_data_mle.clone(), self.r, self.r_packing);

        let input_permuted_packing_builder: InputPackingBuilder<F> =
            InputPackingBuilder::new(self.permuted_input_data_mle.clone(), self.r, self.r_packing);
        ...
        for _ in 0..log2(self.input_len) {
            let prod_builder = SplitProductBuilder::new(input_packed);
            let permuted_prod_builder = SplitProductBuilder::new(input_permuted_packed);
