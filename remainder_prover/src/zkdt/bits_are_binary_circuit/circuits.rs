...
impl<F: FieldExt> GKRCircuit<F> for BinDecomp16BitIsBinaryCircuit<F> {
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        ...
        let output_mle_ref = layers.add_gkr(from_mle(
            ...
            |_mle, id, prefix_bits| {
                ZeroMleRef::new(
                    self.bin_decomp_16_bit_mle.num_iterated_vars(),
                    prefix_bits,
                    id,
                )
            },
...
impl<F: FieldExt> BinDecomp16BitIsBinaryCircuit<F> {
    ...
    pub fn new(bin_decomp_16_bit_mle: DenseMle<F, BinDecomp16Bit<F>>) -> Self {
        Self {
...
impl<F: FieldExt> GKRCircuit<F> for BinDecomp4BitIsBinaryCircuit<F> {
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        ...
        let output_mle_ref = layers.add_gkr(from_mle(
            ...
            |_mle, id, prefix_bits| {
                ZeroMleRef::new(
                    self.multiplicities_bin_decomp_mle_input.num_iterated_vars(),
                    prefix_bits,
                    id,
                )
            },
...
impl<F: FieldExt> BinDecomp4BitIsBinaryCircuit<F> {
    /// Creates a new instance of BinaryRecompCircuit
    pub fn new(multiplicities_bin_decomp_mle_input: DenseMle<F, BinDecomp4Bit<F>>) -> Self {
        Self {
