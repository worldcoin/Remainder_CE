...
mod tests {
    use crate::{
        ...
        zkdt::{
            bits_are_binary_circuit::{
                circuits::{BinDecomp16BitIsBinaryCircuit, BinDecomp4BitIsBinaryCircuit},
                dataparallel_circuits::{
                    BinDecomp16BitIsBinaryCircuitBatched, BinDecomp4BitIsBinaryCircuitBatched,
                    BinDecomp8BitIsBinaryCircuitBatched,
                },
            },
    ...
    fn test_bin_decomp_16_bit_bits_are_binary() {
        ...
        let mut circuit =
            BinDecomp16BitIsBinaryCircuit::<Fr>::new(binary_decomp_diffs_mle_vec[0].clone());

        let mut transcript = PoseidonSponge::new("Bin Decomp Bits Are Binary Transcript");
        let now = Instant::now();
    ...
    fn test_bin_decomp_16_bit_bits_are_binary_batched() {
        ...
        let (
            ...
            (_tree_height, _),
        ) = generate_mles_batch_catboost_single_tree::<Fr>(6, Path::new("upshot_data"));

        let mut circuit = BinDecomp16BitIsBinaryCircuitBatched::new(binary_decomp_diffs_mle_vec);

        let mut transcript = PoseidonSponge::new("Bin Decomp Bits Are Binary Transcript Batched");
        let now = Instant::now();
        let proof = circuit.prove(&mut transcript);
        println!(
            "Proof generated!: Took {} seconds",
        ...
        match proof {
            Ok(proof) => {
                let mut transcript =
                    PoseidonSponge::new("Bin Decomp Bits Are Binary Transcript Batched");
                let result = circuit.verify(&mut transcript, proof);
                if let Err(err) = result {
                    println!("{}", err);
                    panic!();
    ...
    #[test]
    fn test_bin_decomp_8_bit_bits_are_binary_batched() {
        // --- NOTE that this won't work unless we flip the binary decomp endian-ness!!! ---
        // let DummyMles {
        //     dummy_permuted_input_data_mle,
        //     dummy_decision_node_paths_mle,
