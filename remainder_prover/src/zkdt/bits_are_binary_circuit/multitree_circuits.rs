...
use crate::{
    ...
    zkdt::structs::{BinDecomp16Bit, BinDecomp4Bit},
};
...
impl<F: FieldExt> BinDecomp16BitIsBinaryCircuitMultiTree<F> {
    ...
    pub fn new(bin_decomp_16_bit_mle_tree: Vec<DenseMle<F, BinDecomp16Bit<F>>>) -> Self {
        Self {
    ...
    pub fn yield_sub_circuit(
        ...
        self.bin_decomp_16_bit_mle_tree.iter_mut().for_each(|mle| {
            mle.set_prefix_bits(Some(
                mle.get_prefix_bits()
                    .unwrap()
                    .into_iter()
                    .chain(repeat_n(MleIndex::Iterated, num_tree_bits))
                    .collect_vec(),
            ));
        });
        ...
        let output_mle_ref_vec = layers.add_gkr(BatchedLayer::new(
            self.bin_decomp_16_bit_mle_tree
                .iter_mut()
                .map(|bin_decomp_16_bit_mle| {
                    from_mle(
                        bin_decomp_16_bit_mle.clone(),
                        |bin_decomp_16_bit_mle_mle| {
                            let combined_bin_decomp_mle_ref =
                                bin_decomp_16_bit_mle_mle.get_entire_mle_as_mle_ref();
                        ...
                        |_mle, id, prefix_bits| {
                            ZeroMleRef::new(
                                bin_decomp_16_bit_mle.num_iterated_vars(),
                                prefix_bits,
                                id,
                            )
                        },
        ...
        Witness {
            layers,
            output_layers: vec![output_mle_ref.get_enum()],
            input_layers: vec![],
        }
    }
}
...
impl<F: FieldExt> BinDecomp16BitIsBinaryCircuitBatchedMultiTree<F> {
    ...
    ) -> Witness<F, <BinDecomp16BitIsBinaryCircuitBatchedMultiTree<F> as GKRCircuit<F>>::Transcript>
    {
        ...
        let diff_builders_vec = self
            .batched_diff_signed_bin_decomp_tree_mle
            .clone()
            .into_iter()
            .map(|mut batched_diff_signed_bin_decomp_mle| {
                // --- Create the builders for (b_i)^2 - b_i ---
                let diff_builders = batched_diff_signed_bin_decomp_mle
                    .iter_mut()
        ...
        let combined_output_zero_mle_ref = combine_zero_mle_ref(
            res_zero_mle_vec
                .into_iter()
                .map(|res_zero_mle| combine_zero_mle_ref(res_zero_mle))
                .collect_vec(),
        );
...
impl<F: FieldExt> BinDecomp4BitIsBinaryCircuitBatchedMultiTree<F> {
    ...
    ) -> Witness<F, <BinDecomp4BitIsBinaryCircuitBatchedMultiTree<F> as GKRCircuit<F>>::Transcript>
    {
        ...
        let num_dataparallel_bits = log2(num_subcircuit_copies) as usize;
        self.multiplicities_bin_decomp_mle_input_tree_vec
            .iter_mut()
            .for_each(|mle_vec| {
                mle_vec.iter_mut().for_each(|mle| {
                    mle.set_prefix_bits(Some(
