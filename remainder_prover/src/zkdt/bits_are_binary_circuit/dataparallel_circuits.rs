...
use crate::{
    ...
    mle::{dense::DenseMle, zero::ZeroMleRef, Mle, MleIndex, MleRef},
    prover::{
        input_layer::{
    ...
    zkdt::structs::{BinDecomp16Bit, BinDecomp4Bit, BinDecomp8Bit},
};
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        ...
        let mut combined_batched_diff_signed_bin_decomp_mle =
            DenseMle::<F, BinDecomp16Bit<F>>::combine_mle_batch(
                self.batched_diff_signed_bin_decomp_mle.clone(),
            );
        ...
        let diff_builders = self
            ...
            .iter_mut()
            .map(|diff_signed_bin_decomp_mle| {
                diff_signed_bin_decomp_mle.set_prefix_bits(Some(
                    diff_signed_bin_decomp_mle
                        ...
                        .cloned()
                        .chain(
                            combined_batched_diff_signed_bin_decomp_mle
                                .get_prefix_bits()
                                .iter()
                                .flatten()
                                .cloned()
                ...
                from_mle(
                    diff_signed_bin_decomp_mle,
                    |diff_signed_bin_decomp_mle| {
                        let combined_bin_decomp_mle_ref =
                            diff_signed_bin_decomp_mle.get_entire_mle_as_mle_ref();
                        ExpressionStandard::Product(vec![
                            combined_bin_decomp_mle_ref.clone(),
                            combined_bin_decomp_mle_ref.clone(),
                        ]) - ExpressionStandard::Mle(combined_bin_decomp_mle_ref)
                    },
                    |mle, id, prefix_bits| {
            ...
            .collect_vec();
        let combined_output_zero_mle_ref =
            combine_zero_mle_ref(layers.add_gkr(BatchedLayer::new(diff_builders)));

        Witness {
            layers,
            output_layers: vec![combined_output_zero_mle_ref.get_enum()],
...
impl<F: FieldExt> BinDecomp16BitIsBinaryCircuitBatched<F> {
    ...
    pub fn new(batched_diff_signed_bin_decomp_mle: Vec<DenseMle<F, BinDecomp16Bit<F>>>) -> Self {
        Self {
            batched_diff_signed_bin_decomp_mle,
        }
    }
...
impl<F: FieldExt> GKRCircuit<F> for BinDecomp4BitIsBinaryCircuitBatched<F> {
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        ...
        let input_mles: Vec<Box<&mut dyn Mle<F>>> = vec![Box::new(
        ...
        let diff_builders = self
            ...
            .iter_mut()
            .map(|multiplicities_bin_decomp_mle| {
                multiplicities_bin_decomp_mle.set_prefix_bits(Some(
                    multiplicities_bin_decomp_mle
                        ...
                        .cloned()
                        .chain(
                            combined_batched_multiplicities_bin_decomp_mle
                                .get_prefix_bits()
                                .iter()
                                .flatten()
                                .cloned()
...
/// Checks that all of the bits within a `BinDecomp8Bit` are indeed binary
/// via b_i^2 - b_i = 0 (but it's batched)
pub struct BinDecomp8BitIsBinaryCircuitBatched<F: FieldExt> {
...
impl<F: FieldExt> BinDecomp8BitIsBinaryCircuitBatched<F> {
    /// Creates a new instance of BinDecomp16BitIsBinaryCircuitBatched
    pub fn new(
        multiplicities_bin_decomp_mle_input_vec: Vec<DenseMle<F, BinDecomp8Bit<F>>>,
    ) -> Self {
