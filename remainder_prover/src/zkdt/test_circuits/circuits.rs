...
impl<F: FieldExt> GKRCircuit<F> for BatchedFSRandomCircuit<F> {
    ...
    > {
        ...
        let mut combined_batched_mle =
            DenseMle::<F, F>::combine_mle_batch(self.batched_mle.clone());

        // --- Circuit has two input layers: one is the `batched_mle` input layer ---
        let public_input_mles: Vec<Box<&mut dyn Mle<F>>> =
        ...
        let val_mle_input_layer =
            RandomInputLayer::new(transcript, self.val_mle_size, LayerId::Input(1));
        ...
        let random_sub_layer_builders = self
            ...
            .map(|mle| {
                ...
                FSRandomBuilder::new(mle.clone(), val_mle.clone())
            })
            .collect_vec();
        ...
        let self_minus_self_builders = batched_result_mle
            .into_iter()
            .map(|result_mle| SelfMinusSelfBuilder::new(result_mle))
            .collect_vec();
...
impl<F: FieldExt> BatchedFSRandomCircuit<F> {
    /// Constructor
    pub fn new(batched_mle: Vec<DenseMle<F, F>>, val_mle_size: usize) -> Self {
        Self {
            batched_mle,
            val_mle_size,
        }
