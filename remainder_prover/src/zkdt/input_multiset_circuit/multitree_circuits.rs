...
use crate::{
    layer::{
        batched::{combine_mles, combine_zero_mle_ref, unbatch_mles, unflatten_mle, BatchedLayer},
    ...
    zkdt::{
        builders::{
            BitExponentiationBuilderInput, DumbProduct, FSInputPackingBuilder, FSRMinusXBuilder,
            SplitProductBuilderTupleTree,
        },
        structs::{BinDecomp4Bit, BinDecomp8Bit},
    },
};
...
use super::super::{
    builders::{EqualityCheck, ProductBuilder, SplitProductBuilder, SquaringBuilder},
    structs::InputAttribute,
};
...
impl<F: FieldExt> InputMultiSetCircuitMultiTree<F> {
    pub fn yield_sub_circuit(&mut self) -> Witness<F, PoseidonSponge<F>> {
        ...
        self.permuted_input_data_mle_vec_tree
            .iter_mut()
            .for_each(|mle_vec| {
        ...
        self.multiplicities_bin_decomp_mle_input_vec
            .iter_mut()
            .for_each(|mle| {
                mle.set_prefix_bits(Some(
                    mle.get_prefix_bits()
        ...
        let multiplicities_bin_decomp_mle_input_vec =
            self.multiplicities_bin_decomp_mle_input_vec.clone();
        ...
        let prev_prod_builders = BatchedLayer::new(
            input_packed_vec
                .iter()
                .zip(multiplicities_bin_decomp_mle_input_vec.iter())
        ...
        let r_minus_x_square_builders = BatchedLayer::new(
            input_packed_vec
                ...
                .map(|r_minus_x_power| {
                    ...
                    SquaringBuilder::new(r_minus_x_power)
                })
                .collect_vec(),
        );
        ...
        let prev_prod_builders = BatchedLayer::new(
            r_minus_x_power_vec
                .iter()
                .zip(multiplicities_bin_decomp_mle_input_vec.iter())
                .map(|(r_minus_x_power, multiplicities_bin_decomp_mle)| {
                    BitExponentiationBuilderInput::new(
        ...
        let prod_builders = BatchedLayer::new(
            curr_prod_vec
                .iter()
                .zip(prev_prod_vec.iter())
                .map(|(curr_prod, prev_prod)| {
                    ProductBuilder::new(curr_prod.clone(), prev_prod.clone())
                })
                .collect_vec(),
        );

        let layer_4_builders = r_minus_x_square_builders
        ...
        let layer_5_builders = r_minus_x_square_builders
            .concat(curr_prod_builders)
            .concat_with_padding(prod_builders, Padding::Right(1));
        ...
        let layer_6_builders = r_minus_x_square_builders
            .concat(curr_prod_builders)
            .concat_with_padding(prod_builders, Padding::Right(1));
        ...
        let layer_7_builders = r_minus_x_square_builders
            .concat(curr_prod_builders)
            .concat_with_padding(prod_builders, Padding::Right(1));
        ...
        let layer_8_builders = r_minus_x_square_builders
            .concat(curr_prod_builders)
            .concat_with_padding(prod_builders, Padding::Right(1));
        ...
        for _ in 0..log2(input_len) {
            let split_product_builders = BatchedLayer::new(
                exponentiated_input_vec
                    .into_iter()
                    .map(|exponentiated_input| SplitProductBuilder::new(exponentiated_input))
                    .collect(),
            );
        ...
        let path_packing_builders_vec = self
            .permuted_input_data_mle_vec_tree
            .iter()
        ...
        let permuted_input_packed_vecs =
            layers.add_gkr(BatchedLayer::new(path_packing_builders_vec));
        let permuted_product_vecs = permuted_input_packed_vecs;

        let mut permuted_product_huge = unbatch_mles(
            permuted_product_vecs
                .into_iter()
                .map(|vec| unbatch_mles(vec))
                .collect_vec(),
        );
        ...
        for _ in 0..(self.permuted_input_data_mle_vec_tree[0][0].num_iterated_vars() - 1) {
            let permuted_product_tuple =
                permuted_product_huge.split_tree(1 << num_dataparallel_bits);
        ...
        let difference_builder = EqualityCheck::new(unbatched_expo, permuted_product_huge);
