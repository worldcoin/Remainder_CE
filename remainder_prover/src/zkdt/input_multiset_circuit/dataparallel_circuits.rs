...
use crate::{
    ...
    zkdt::{
        builders::{BitExponentiationBuilderInput, FSInputPackingBuilder, FSRMinusXBuilder},
...
impl<F: FieldExt> GKRCircuit<F> for InputMultiSetCircuit<F> {
    ...
    fn synthesize_and_commit(
        ...
        for multiplicities_bin_decomp_mle_input in
            self.multiplicities_bin_decomp_mle_input_vec.iter_mut()
        {
            multiplicities_bin_decomp_mle_input.set_prefix_bits(Some(
                multiplicities_bin_decomp_mle_input_vec_combined
                    .get_prefix_bits()
                    .unwrap()
                    .into_iter()
        ...
        let input_packing_builders = BatchedLayer::new(
            self.input_data_mle_vec
                ...
                .map(|input_data_mle| {
                    ...
                    FSInputPackingBuilder::new(input_data_mle, r_mle.clone(), r_packing_mle.clone())
                })
                .collect_vec(),
        );
        ...
        let r_minus_x_builders = BatchedLayer::new(
            input_packed_vec
                ...
                .map(|input_packed| {
                    ...
                    FSRMinusXBuilder::new(input_packed, r_mle_another.clone())
                })
                .collect_vec(),
        );
        ...
        let path_packing_builders = BatchedLayer::new(
            self.permuted_input_data_mle_vec
                ...
                .map(|permuted_input_data_mle| {
                    ...
                    permuted_input_data_mle.set_prefix_bits(Some(
                        permuted_input_data_mle_vec_combined
                            .get_prefix_bits()
                            .unwrap()
                            .into_iter()
        ...
        let r_minus_x_path_builders = BatchedLayer::new(
            path_packed_vec
                .iter()
                .map(|path_packed| {
                    FSRMinusXBuilder::new(path_packed.clone(), r_mle_another.clone())
                })
                .collect_vec(),
        );
        ...
        for _ in 0..log2(permuted_input_len) {
            let split_product_builders = BatchedLayer::new(
                path_product_vec
                    .into_iter()
                    .map(|path_product| SplitProductBuilder::new(path_product))
                    .collect(),
            );
        ...
        // layer 18
        let difference_builder =
            EqualityCheck::new_batched(exponentiated_input_vec, path_product_vec);
