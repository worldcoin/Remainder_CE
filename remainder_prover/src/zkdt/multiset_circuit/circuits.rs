...
use crate::{
    layer::{
        batched::{unbatch_mles, BatchedLayer},
...
pub(crate) struct FSMultiSetCircuit<F: FieldExt> {
    ...
    pub decision_node_paths_mle_vec: Vec<DenseMle<F, DecisionNode<F>>>, // batched
    pub leaf_node_paths_mle_vec: Vec<DenseMle<F, LeafNode<F>>>,         // batched
...
impl<F: FieldExt> GKRCircuit<F> for FSMultiSetCircuit<F> {
    ...
    fn synthesize_and_commit(
        ...
        let mut dummy_decision_node_paths_mle_vec_combined =
            DenseMle::<F, DecisionNode<F>>::combine_mle_batch(
                self.decision_node_paths_mle_vec.clone(),
            );
        let mut dummy_leaf_node_paths_mle_vec_combined =
            DenseMle::<F, LeafNode<F>>::combine_mle_batch(self.leaf_node_paths_mle_vec.clone());
        ...
        let leaf_packing_builder =
            FSLeafPackingBuilder::new(dummy_leaf_nodes_mle, r_mle.clone(), r_packing_mle.clone());
        ...
        let r_minus_x_square_builders =
            r_minus_x_square_builder_decision.concat(r_minus_x_square_builder_leaf);
        ...
        let (
            (mut prev_prod_decision, mut prev_prod_leaf),
            (r_minus_x_power_decision, r_minus_x_power_leaf),
        ) = layers.add_gkr(layer_2_builders);
        ...
        let r_minus_x_square_builder_decision = SquaringBuilder::new(r_minus_x_power_decision);
        let r_minus_x_square_builder_leaf = SquaringBuilder::new(r_minus_x_power_leaf);
        let r_minus_x_square_builders =
            r_minus_x_square_builder_decision.concat(r_minus_x_square_builder_leaf);

        let layer_3_builders = pre_prod_builders.concat(r_minus_x_square_builders);
        let (
            (mut curr_prod_decision, mut curr_prod_leaf),
            (mut r_minus_x_power_decision, mut r_minus_x_power_leaf),
        ) = layers.add_gkr(layer_3_builders);
        ...
        for i in 2..15 {
            ...
            let r_minus_x_square_builder_leaf = SquaringBuilder::new(r_minus_x_power_leaf.clone());
            let r_minus_x_square_builders =
                r_minus_x_square_builder_decision.concat(r_minus_x_square_builder_leaf);
            ...
            // layer 4, part 3
            let prod_builder_decision = ProductBuilder::new(curr_prod_decision, prev_prod_decision);
            ...
            let layer_i_builders = r_minus_x_square_builders
                .concat(curr_prod_builders)
                .concat_with_padding(prod_builders, Padding::Right(1));
            ...
            (
                ...
                (prev_prod_decision, prev_prod_leaf),
            ) = layers.add_gkr(layer_i_builders);
        }
        ...
        let ((curr_prod_decision, curr_prod_leaf), (prev_prod_decision, prev_prod_leaf)) =
            layers.add_gkr(layer_17_builders);
        ...
        for _ in 0..tree_height - 1 {
            // layer 20, or i+20
            let prod_builder_decision = SplitProductBuilder::new(exponentiated_decision);
        ...
        let prod_builder_nodes = ProductBuilder::new(exponentiated_decision, exponentiated_leaf);
        ...
        let bit_difference = self.decision_node_paths_mle_vec[0].num_iterated_vars()
            - self.leaf_node_paths_mle_vec[0].num_iterated_vars();
        ...
        let decision_path_packing_builder = BatchedLayer::new(
            self.decision_node_paths_mle_vec
                .iter()
                .map(|decision_node_mle| {
                    let mut decision_node_mle = decision_node_mle.clone();
                    decision_node_mle.set_prefix_bits(Some(
                        dummy_decision_node_paths_mle_vec_combined
                            .get_prefix_bits()
                            .unwrap()
                            .into_iter()
        ...
        let leaf_path_packing_builder = BatchedLayer::new(
            self.leaf_node_paths_mle_vec
                .iter()
                .map(|leaf_node_mle| {
                    let mut leaf_node_mle = leaf_node_mle.clone();
                    leaf_node_mle.set_prefix_bits(Some(
                        dummy_leaf_node_paths_mle_vec_combined
                            .get_prefix_bits()
                            .unwrap()
                            .into_iter()
                            .chain(repeat_n(MleIndex::Iterated, batch_bits))
        ...
        let path_packing_builders = decision_path_packing_builder.concat_with_padding(
            leaf_path_packing_builder,
        ...
        let path_product_builder = ProductBuilder::new(vector_x_decision, vector_x_leaf);
...
impl<F: FieldExt> FSMultiSetCircuit<F> {
    pub fn yield_sub_circuit(&mut self) -> Witness<F, PoseidonSponge<F>> {
        ...
        let mut multiplicities_bin_decomp_mle_decision =
            self.multiplicities_bin_decomp_mle_decision.clone();
        multiplicities_bin_decomp_mle_decision.set_prefix_bits(
            self.multiplicities_bin_decomp_mle_decision
                .get_prefix_bits(),
        );
        ...
        let mut multiplicities_bin_decomp_mle_leaf =
            self.multiplicities_bin_decomp_mle_leaf.clone();
        multiplicities_bin_decomp_mle_leaf
            .set_prefix_bits(self.multiplicities_bin_decomp_mle_leaf.get_prefix_bits());
        let prev_prod_builder_leaf = BitExponentiationBuilderCatBoost::new(
        ...
        // evaluated at a random challenge point ---
        let exponentiated_nodes =
            layers.add::<_, EmptyLayer<F, PoseidonSponge<F>>>(prod_builder_nodes);
        ...
        // --- This is our final answer \prod_{i = 0}^n (r - x_i) for the path node characteristic polynomial
        // evaluated at the same random challenge point ---
        let path_product = layers.add::<_, EmptyLayer<F, PoseidonSponge<F>>>(path_product_builder);

        let difference_builder = EqualityCheck::new(exponentiated_nodes, path_product);

        let circuit_output = layers.add::<_, EmptyLayer<F, PoseidonSponge<F>>>(difference_builder);
