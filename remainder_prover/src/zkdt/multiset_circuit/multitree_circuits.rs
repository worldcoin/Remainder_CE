use ark_std::log2;
use itertools::{multizip, repeat_n, Itertools};

use crate::prover::{GKRCircuit, GKRError, Layers, Witness};
use crate::{
    layer::{
    ...
    zkdt::builders::{
        BitExponentiationBuilderCatBoost, FSDecisionPackingBuilder, FSLeafPackingBuilder,
        FSRMinusXBuilder,
    },
};
...
use super::{
    super::{
        builders::{
            DecisionPackingBuilder, EqualityCheck, LeafPackingBuilder, ProductBuilder,
            RMinusXBuilder, SplitProductBuilder, SquaringBuilder,
        },
        structs::{BinDecomp16Bit, DecisionNode, LeafNode},
    },
    legacy_circuits::MultiSetCircuit,
};
...
impl<F: FieldExt> FSMultiSetCircuitMultiTree<F> {
    pub fn yield_sub_circuit(&mut self) -> Witness<F, PoseidonSponge<F>> {
        ...
        self.decision_nodes_mle_tree.iter_mut().for_each(|mle| {
            mle.set_prefix_bits(Some(
                mle.get_prefix_bits()
                    .unwrap()
                    .into_iter()
                    .chain(repeat_n(MleIndex::Iterated, tree_bits))
                    .collect_vec(),
            ));
        });
        self.leaf_nodes_mle_tree.iter_mut().for_each(|mle| {
            mle.set_prefix_bits(Some(
                mle.get_prefix_bits()
                    .unwrap()
                    .into_iter()
        ...
        self.multiplicities_bin_decomp_mle_decision_tree
            .iter_mut()
            .for_each(|mle| {
                mle.set_prefix_bits(Some(
                    mle.get_prefix_bits()
        ...
        self.multiplicities_bin_decomp_mle_leaf_tree
            .iter_mut()
            .for_each(|mle| {
                mle.set_prefix_bits(Some(
                    mle.get_prefix_bits()
        ...
        self.decision_node_paths_mle_vec_tree
            .iter_mut()
            .for_each(|mle_vec| {
                mle_vec.iter_mut().for_each(|mle| {
                    mle.set_prefix_bits(Some(
                        mle.get_prefix_bits()
                            ...
                            .chain(repeat_n(
                                MleIndex::Iterated,
                                num_dataparallel_bits + tree_bits,
                            ))
                            .collect_vec(),
                    ));
                })
        ...
        self.leaf_node_paths_mle_vec_tree
            .iter_mut()
            .for_each(|mle_vec| {
                mle_vec.iter_mut().for_each(|mle| {
                    mle.set_prefix_bits(Some(
        ...
        let (decision_packing_builder_vec, leaf_packing_builder_vec): (Vec<_>, Vec<_>) = self
            .decision_nodes_mle_tree
            .clone()
            .into_iter()
            .zip(self.leaf_nodes_mle_tree.clone().into_iter())
            .map(|(decision_nodes_mle, leaf_nodes_mle)| {
        ...
        let (decision_packed_vec, leaf_packed_vec): (Vec<_>, Vec<_>) = layers.add_gkr(
            BatchedLayer::new(decision_packing_builder_vec)
                .concat(BatchedLayer::new(leaf_packing_builder_vec)),
        );
        ...
        let (
            prev_prod_builder_decision_vec,
            prev_prod_builder_leaf_vec,
            r_minus_x_square_builder_decision_vec,
            ...
            .map(
                |(
                    ...
                    let r_minus_x_square_builder_decision = SquaringBuilder::new(decision_packed);
                    let r_minus_x_square_builder_leaf = SquaringBuilder::new(leaf_packed);
                    ...
                    (
                        prev_prod_builder_decision,
                        prev_prod_builder_leaf,
                        r_minus_x_square_builder_decision,
                        r_minus_x_square_builder_leaf,
                    )
                },
            )
            .multiunzip();

        let concat_builder_layer_2 = (BatchedLayer::new(prev_prod_builder_decision_vec)
            .concat(BatchedLayer::new(prev_prod_builder_leaf_vec)))
        .concat(
            BatchedLayer::new(r_minus_x_square_builder_decision_vec)
                .concat(BatchedLayer::new(r_minus_x_square_builder_leaf_vec)),
        );

        // --- For number-of-layers efficiency, we combine the above layers into a single GKR layer ---

        let (
            (mut prev_prod_decision_vec, mut prev_prod_leaf_vec),
            (r_minus_x_power_decision_vec, r_minus_x_power_leaf_vec),
        ): ((Vec<_>, Vec<_>), (Vec<_>, Vec<_>)) = layers.add_gkr(concat_builder_layer_2);
        ...
        let (
            ...
            r_minus_x_square_builder_decision_vec,
            r_minus_x_square_builder_leaf_vec,
        ): (Vec<_>, Vec<_>, Vec<_>, Vec<_>) = multizip((
            self.multiplicities_bin_decomp_mle_decision_tree.iter(),
            self.multiplicities_bin_decomp_mle_leaf_tree.iter(),
            r_minus_x_power_decision_vec.iter(),
            r_minus_x_power_leaf_vec.iter(),
        ))
        .map(
            |(
                ...
                let r_minus_x_square_builder_decision =
                    SquaringBuilder::new(r_minus_x_power_decision.clone());
                let r_minus_x_square_builder_leaf =
                    SquaringBuilder::new(r_minus_x_power_leaf.clone());
                // let r_minus_x_square_builders = r_minus_x_square_builder_decision.concat(r_minus_x_square_builder_leaf);
        ...
        let layer_3_builders_concat = BatchedLayer::new(prev_prod_builder_decision_vec)
            .concat(BatchedLayer::new(prev_prod_builder_leaf_vec))
            .concat(
                BatchedLayer::new(r_minus_x_square_builder_decision_vec)
                    .concat(BatchedLayer::new(r_minus_x_square_builder_leaf_vec)),
        ...
        let (
            (mut curr_prod_decision_vec, mut curr_prod_leaf_vec),
            (mut r_minus_x_power_decision_vec, mut r_minus_x_power_leaf_vec),
        ): ((Vec<_>, Vec<_>), (Vec<_>, Vec<_>)) = layers.add_gkr(layer_3_builders_concat);
        ...
        for i in 2..15 {
            let (
                ...
                prod_builder_leaf_vec,
            ): (Vec<_>, Vec<_>, Vec<_>, Vec<_>, Vec<_>, Vec<_>) = multizip((
                r_minus_x_power_decision_vec.iter(),
                r_minus_x_power_leaf_vec.iter(),
                curr_prod_decision_vec.into_iter(),
                curr_prod_leaf_vec.into_iter(),
                prev_prod_decision_vec.into_iter(),
                prev_prod_leaf_vec.into_iter(),
                self.multiplicities_bin_decomp_mle_decision_tree.iter(),
                self.multiplicities_bin_decomp_mle_leaf_tree.iter(),
            ))
            .map(
                |(
                    ...
                    // layer 4, part 3
                    let prod_builder_decision =
                        ProductBuilder::new(curr_prod_decision, prev_prod_decision);

                    let prod_builder_leaf = ProductBuilder::new(curr_prod_leaf, prev_prod_leaf);
                    ...
                    (
                        r_minus_x_square_builder_decision,
                        r_minus_x_square_builder_leaf,
                        curr_prod_builder_decision,
                        curr_prod_builder_leaf,
                        prod_builder_decision,
                        prod_builder_leaf,
                    )
            ...
            let r_minus_x_square_builders_vec =
                BatchedLayer::new(r_minus_x_square_builder_decision_vec)
                    .concat(BatchedLayer::new(r_minus_x_square_builder_leaf_vec));
            let curr_prod_builders_vec = BatchedLayer::new(curr_prod_builder_decision_vec)
                .concat(BatchedLayer::new(curr_prod_builder_leaf_vec));
            let prod_builders_vec = BatchedLayer::new(prod_builder_decision_vec)
                .concat(BatchedLayer::new(prod_builder_leaf_vec));
            let layer_i_builders_vec = r_minus_x_square_builders_vec
                .concat(curr_prod_builders_vec)
                .concat_with_padding(prod_builders_vec, Padding::Right(1));
            ...
            (
                ...
                (prev_prod_decision_vec, prev_prod_leaf_vec),
            ) = layers.add_gkr(layer_i_builders_vec);
        }
        ...
        let (
            ...
            r_minus_x_power_decision_vec.into_iter(),
            r_minus_x_power_leaf_vec.into_iter(),
            self.multiplicities_bin_decomp_mle_decision_tree.iter(),
            self.multiplicities_bin_decomp_mle_leaf_tree.iter(),
            curr_prod_decision_vec.into_iter(),
            curr_prod_leaf_vec.into_iter(),
        ...
        let curr_prod_builders = BatchedLayer::new(curr_prod_builder_decision_vec)
            .concat(BatchedLayer::new(curr_prod_builder_leaf_vec));
        let prod_builders = BatchedLayer::new(prod_builder_decision_vec)
            .concat(BatchedLayer::new(prod_builder_leaf_vec));
        ...
        let (
            ...
            layers.add_gkr(curr_prod_builders.concat(prod_builders));

        // layer 18

        let (prod_builder_decision_vec, prod_builder_leaf_vec): (Vec<_>, Vec<_>) = multizip((
            prev_prod_decision_vec,
            prev_prod_leaf_vec,
            curr_prod_decision_vec,
            curr_prod_leaf_vec,
        ))
        .map(
        ...
        let (prev_prod_decision_vec, prev_prod_leaf_vec): (Vec<_>, Vec<_>) = layers.add_gkr(
            BatchedLayer::new(prod_builder_decision_vec)
                .concat(BatchedLayer::new(prod_builder_leaf_vec)),
        );
        ...
        let tree_height =
            (1 << (self.decision_node_paths_mle_vec_tree[0][0].num_iterated_vars() - 2)) + 1;
        ...
        for _ in 0..tree_height - 1 {
            let (prod_builders_decision_vec, prod_builders_leaf_vec): (Vec<_>, Vec<_>) =
                exponentiated_decision_vec
                    .into_iter()
                    .zip(exponentiated_leaf_vec)
                    .map(|(exponentiated_decision, exponentiated_leaf)| {
                        // layer 20, or i+20
                        let prod_builder_decision =
                            SplitProductBuilder::new(exponentiated_decision);
                        let prod_builder_leaf = SplitProductBuilder::new(exponentiated_leaf);
            ...
            (exponentiated_decision_vec, exponentiated_leaf_vec) = layers.add_gkr(
                BatchedLayer::new(prod_builders_decision_vec)
                    .concat(BatchedLayer::new(prod_builders_leaf_vec)),
            );
        }

        let prod_builder_nodes_vec = exponentiated_decision_vec
            .into_iter()
            .zip(exponentiated_leaf_vec.into_iter())
            .map(|(exponentiated_decision, exponentiated_leaf)| {
                let prod_builder_nodes =
                    ProductBuilder::new(exponentiated_decision, exponentiated_leaf);
                prod_builder_nodes
            })
            .collect_vec();
        ...
        println!(
            "Nodes exponentiated, number of layers {:?}",
            layers.next_layer_id()
        );

        let bit_difference = self.decision_node_paths_mle_vec_tree[0][0].num_iterated_vars()
            - self.leaf_node_paths_mle_vec_tree[0][0].num_iterated_vars();
        ...
        let (decision_path_packing_builder_vec, leaf_path_packing_builder_vec): (Vec<_>, Vec<_>) =
            self.decision_node_paths_mle_vec_tree
                .clone()
                .into_iter()
                .zip(self.leaf_node_paths_mle_vec_tree.clone().into_iter())
                .map(|(decision_node_paths_mle_vec, leaf_node_paths_mle_vec)| {
                    let decision_path_packing_builder = BatchedLayer::new(
                        decision_node_paths_mle_vec
                            .iter()
                            .map(|decision_node_mle| {
        ...
        let (decision_path_packed_vec, leaf_path_packed_vec): (Vec<_>, Vec<_>) = layers.add_gkr(
            BatchedLayer::new(decision_path_packing_builder_vec).concat_with_padding(
                BatchedLayer::new(leaf_path_packing_builder_vec),
                Padding::Right(bit_difference - 1),
            ),
        );
        ...
        let mut vector_x_decision_vec = decision_path_packed_vec
            .into_iter()
            .map(|decision_path_packed| unbatch_mles(decision_path_packed))
            .collect_vec();
        let mut vector_x_leaf_vec = leaf_path_packed_vec
            .into_iter()
            .map(|leaf_path_packed| unbatch_mles(leaf_path_packed))
            .collect_vec();

        for _ in 0..vector_x_decision_vec[0].num_iterated_vars() {
            vector_x_decision_vec = layers.add_gkr(BatchedLayer::new(
                vector_x_decision_vec
                    .into_iter()
                    .map(|vector_x_decision| SplitProductBuilder::new(vector_x_decision))
                    .collect_vec(),
            ));
        }

        for _ in 0..vector_x_leaf_vec[0].num_iterated_vars() {
            vector_x_leaf_vec = layers.add_gkr(BatchedLayer::new(
                vector_x_leaf_vec
                    .into_iter()
                    .map(|vector_x_leaf| SplitProductBuilder::new(vector_x_leaf))
                    .collect_vec(),
            ));
        }

        let path_product_builder_vec = vector_x_decision_vec
            .into_iter()
            .zip(vector_x_leaf_vec.into_iter())
            .map(|(vector_x_decision, vector_x_leaf)| {
                ProductBuilder::new(vector_x_decision, vector_x_leaf)
            })
            .collect_vec();
        ...
        let difference_builder_vec = exponentiated_nodes_vec
            .into_iter()
            .zip(path_product_vec.into_iter())
            .map(|(exponentiated_nodes, path_product)| {
                let difference_builder = EqualityCheck::new(exponentiated_nodes, path_product);
                difference_builder
            })
            .collect_vec();

        let circuit_output_vec = layers
            .add::<_, EmptyLayer<F, PoseidonSponge<F>>>(BatchedLayer::new(difference_builder_vec));
