...
use super::circuit_builders::{
    ...
    PrevNodeRightBuilderDecision, SignBit, SubtractBuilder, TwoTimesBuilder,
};
...
impl<F: FieldExt> GKRCircuit<F> for PathCheckCircuitBatched<F> {
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Sponge> {
        ...
        let mut combined_leaf =
            DenseMle::<F, LeafNode<F>>::combine_mle_batch(self.batched_leaf_node_paths_mle.clone());
        let mut combined_bit = DenseMle::<F, BinDecomp16Bit<F>>::combine_mle_batch(
            self.batched_bin_decomp_diff_mle.clone(),
        );

        let input_mles: Vec<Box<&mut dyn Mle<F>>> = vec![
            Box::new(&mut combined_decision),
            Box::new(&mut combined_leaf),
        ...
        self.batched_bin_decomp_diff_mle
            .iter_mut()
            .for_each(|bin_decomp_mle| {
        ...
        self.batched_decision_node_paths_mle
            .iter_mut()
            .for_each(|dec_mle| {
        ...
        self.batched_leaf_node_paths_mle
            .iter_mut()
            .for_each(|leaf_mle| {
        ...
        let two_times_plus_sign_builders = two_times_things
            ...
            .map(|(two_times, sign_bit)| SubtractBuilder::new(two_times.clone(), sign_bit.clone()))
            .collect_vec();
        ...
        let curr_node_decision_builders = self
            .batched_decision_node_paths_mle
            .iter()
        ...
        let curr_node_leaf_builders = self
        ...
        let nonzero_gates_add_decision = decision_add_wiring_from_size(
            1 << (flattened_two_times_plus_sign.num_iterated_vars() - num_dataparallel_bits),
        );
