...
use crate::{
    ...
    layer::{empty_layer::EmptyLayer, LayerId},
    ...
    zkdt::{
        builders::ConcatBuilder,
        structs::{BinDecomp16Bit, DecisionNode, LeafNode},
    },
};

use super::circuit_builders::{
    CurrNodeBuilderDecision, CurrNodeBuilderLeaf, OneMinusSignBit, PrevNodeLeftBuilderDecision,
    PrevNodeRightBuilderDecision, SignBit, SignBitProductBuilder,
};

/// Helper!
pub fn create_wiring_from_size(size: usize) -> Vec<(usize, usize, usize)> {
    let mut gates = (0..(size - 1))
        .map(|idx| (idx, 2 * (idx + 1), idx))
        .collect_vec();
...
pub fn decision_mul_wiring_from_size(size: usize) -> Vec<(usize, usize, usize)> {
    (0..(size - 1)).map(|idx| (idx, idx, idx)).collect_vec()
}
...
impl<F: FieldExt> GKRCircuit<F> for PathCheckCircuit<F> {
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        ...
        let input_mles: Vec<Box<&mut dyn Mle<F>>> = vec![
            ...
            Box::new(&mut self.leaf_node_paths_mle),
            Box::new(&mut self.bin_decomp_diff_mle),
        ];
        let input_layer_builder = InputLayerBuilder::<F>::new(input_mles, None, LayerId::Input(0));
        let input_layer = input_layer_builder
        ...
        let prev_node_left_builder =
            PrevNodeLeftBuilderDecision::new(self.decision_node_paths_mle.clone());

        let prev_node_right_builder =
            PrevNodeRightBuilderDecision::new(self.decision_node_paths_mle.clone());

        let curr_node_decision_builder =
            CurrNodeBuilderDecision::new(self.decision_node_paths_mle.clone());
        ...
        let curr_node_decision_leaf_mle_ref =
            layers.add_gkr(curr_node_decision_leaf_builder).mle_ref();
        ...
        let nonzero_gates =
            create_wiring_from_size(1 << (prev_node_left_mle_ref.num_vars() - self.num_copy));

        let res_negative = layers.add_gate(
            nonzero_gates.clone(),
            curr_node_decision_leaf_mle_ref.clone(),
            prev_node_left_mle_ref,
            None,
            BinaryOperation::Add,
        );
        let res_positive = layers.add_gate(
            nonzero_gates,
            curr_node_decision_leaf_mle_ref,
            prev_node_right_mle_ref,
            None,
            BinaryOperation::Add,
        );

        let sign_bit_sum_builder: SignBitProductBuilder<F> =
...
impl<F: FieldExt> GKRCircuit<F> for PathMulCheckCircuit<F> {
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        ...
        let nonzero_gates_add_decision =
            decision_add_wiring_from_size(1 << (prev_node_left_mle_ref.num_vars()));
        let nonzero_gates_add_leaf =
            leaf_add_wiring_from_size(1 << (prev_node_left_mle_ref.num_vars()));

        let res_negative_dec = layers.add_gate(
            nonzero_gates_add_decision.clone(),
            curr_decision_mle_ref.clone(),
            prev_node_left_mle_ref.clone(),
            None,
            BinaryOperation::Add,
        );
        let res_positive_dec = layers.add_gate(
            nonzero_gates_add_decision,
            curr_decision_mle_ref,
            prev_node_right_mle_ref.clone(),
            None,
            BinaryOperation::Add,
        );

        let res_negative_leaf = layers.add_gate(
            nonzero_gates_add_leaf.clone(),
            prev_node_left_mle_ref,
            curr_leaf_mle_ref.clone(),
            None,
            BinaryOperation::Add,
        );
        let res_positive_leaf = layers.add_gate(
            nonzero_gates_add_leaf,
            prev_node_right_mle_ref,
            curr_leaf_mle_ref,
            None,
            BinaryOperation::Add,
        ...
        let nonzero_gates_mul_decision =
            decision_mul_wiring_from_size(1 << (pos_sign_bits.num_iterated_vars()));
        let nonzero_gates_mul_leaf =
            leaf_mul_wiring_from_size(1 << (pos_sign_bits.num_iterated_vars()));

        let dec_pos_prod = layers.add_gate(
            nonzero_gates_mul_decision.clone(),
            pos_sign_bits.mle_ref(),
            res_positive_dec.mle_ref(),
            None,
            BinaryOperation::Mul,
        );
        let dec_neg_prod = layers.add_gate(
            nonzero_gates_mul_decision,
            neg_sign_bits.mle_ref(),
            res_negative_dec.mle_ref(),
            None,
            BinaryOperation::Mul,
        );
        let leaf_pos_prod = layers.add_gate(
            nonzero_gates_mul_leaf.clone(),
            pos_sign_bits.mle_ref(),
            res_positive_leaf.mle_ref(),
            None,
            BinaryOperation::Mul,
        );
        let leaf_neg_prod = layers.add_gate(
            nonzero_gates_mul_leaf,
            neg_sign_bits.mle_ref(),
            res_negative_leaf.mle_ref(),
            None,
            BinaryOperation::Mul,
        );
        ...
        let witness: Witness<F, Self::Transcript> = Witness {
            ...
            output_layers: vec![
                ...
                dec_neg_prod.mle_ref().get_enum(),
                leaf_pos_prod.mle_ref().get_enum(),
                leaf_neg_prod.mle_ref().get_enum(),
            ],
            input_layers: vec![input_layer],
        };
