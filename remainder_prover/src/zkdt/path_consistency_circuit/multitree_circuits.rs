...
use crate::{
    ...
    mle::{dense::DenseMle, mle_enum::MleEnum, Mle, MleIndex, MleRef},
    prover::{
        input_layer::{
...
use super::{
    circuit_builders::{
        CurrNodeBuilderDecision, CurrNodeBuilderLeaf, OneMinusSignBit, SubtractBuilder,
        TwoTimesBuilder,
    },
    circuits::{decision_add_wiring_from_size, leaf_add_wiring_from_size},
};
...
impl<F: FieldExt> GKRCircuit<F> for PathCheckCircuitBatchedNoMulMultiTree<F> {
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Sponge> {
        ...
        let decision_node_paths_mle_combined_per_tree = self
            ...
            .collect_vec();
        let mut decision_node_paths_combined =
            DenseMle::<F, F>::combine_mle_batch(decision_node_paths_mle_combined_per_tree);

        let leaf_node_paths_mle_combined_per_tree = self
            ...
            .collect_vec();
        let mut leaf_node_paths_combined =
            DenseMle::<F, F>::combine_mle_batch(leaf_node_paths_mle_combined_per_tree);

        let bin_decomp_diff_combined_per_tree = self
            .batched_bin_decomp_diff_mle_vec
            .clone()
            .into_iter()
            .map(|bin_decomp_mle_vec| {
                DenseMle::<F, BinDecomp16Bit<F>>::combine_mle_batch(bin_decomp_mle_vec)
            })
            .collect_vec();
        let mut bin_decomp_diffs_combined =
            DenseMle::<F, F>::combine_mle_batch(bin_decomp_diff_combined_per_tree);
            ...
            .for_each(|decision_node_paths_mle_vec| {
                decision_node_paths_mle_vec
                    ...
                    .for_each(|decision_node_paths_mle| {
                        decision_node_paths_mle.set_prefix_bits(Some(
                            decision_node_paths_combined
                                .get_prefix_bits()
                                .unwrap()
                                .into_iter()
                                .chain(repeat_n(
            ...
            .for_each(|leaf_node_paths_mle_vec| {
                leaf_node_paths_mle_vec
                    ...
                    .for_each(|leaf_node_paths_mle| {
                        leaf_node_paths_mle.set_prefix_bits(Some(
                            leaf_node_paths_combined
                                .get_prefix_bits()
                                .unwrap()
                                .into_iter()
                                .chain(repeat_n(
            ...
            .for_each(|binary_decomp_diffs_mle_vec| {
                binary_decomp_diffs_mle_vec
                    ...
                    .for_each(|binary_decomp_diffs_mle| {
                        binary_decomp_diffs_mle.set_prefix_bits(Some(
                            bin_decomp_diffs_combined
                                .get_prefix_bits()
                                .unwrap()
                                .into_iter()
                                .chain(repeat_n(
        ...
        let pos_builders_vec = self
            .batched_bin_decomp_diff_mle_vec
            .iter()
            .map(|batched_bin_decomp_diff_mle| {
                let pos_builders = batched_bin_decomp_diff_mle
                    .iter()
                    .map(|bin_decomp_mle| OneMinusSignBit::new(bin_decomp_mle.clone()))
                    .collect_vec();
        ...
        let two_times_batched_builder_vec = self
            .batched_decision_node_paths_mle_vec
            .iter()
            .map(|batched_decision_node_paths_mle| {
                let two_times_builders = batched_decision_node_paths_mle
                    .iter()
                    .map(|dec_mle| TwoTimesBuilder::new(dec_mle.clone()))
                    .collect_vec();
        ...
        let two_times_plus_sign_builders_vec = two_times_things_vec
            .iter()
            .zip(pos_sign_bits_vec.iter())
            .map(|(two_times_things, pos_sign_bits)| {
                let two_times_plus_sign_builders = two_times_things
                    .iter()
                    .zip(pos_sign_bits.iter())
                    .map(|(two_times, sign_bit)| {
        ...
        let two_times_plus_sign_vals_vec =
            layers.add_gkr(BatchedLayer::new(two_times_plus_sign_builders_vec)); // ID 2

        let curr_node_decision_builders_vec = self
            .batched_decision_node_paths_mle_vec
            .iter()
            .map(|batched_decision_node_paths_mle| {
                let curr_node_decision_builders = batched_decision_node_paths_mle
                    .iter()
                    .map(|dec_mle| CurrNodeBuilderDecision::new(dec_mle.clone()))
                    .collect_vec();
        ...
        let curr_node_leaf_builders_vec = self
            .batched_leaf_node_paths_mle_vec
            .iter()
            .map(|batched_leaf_node_paths_mle| {
                let curr_node_leaf_builders = batched_leaf_node_paths_mle
                    .iter()
                    .map(|leaf_mle| CurrNodeBuilderLeaf::new(leaf_mle.clone()))
                    .collect_vec();
        ...
        let flattened_curr_dec_vec = curr_decision_vec
            .into_iter()
            .map(|curr_decision| unbatch_mles(curr_decision))
            .collect_vec();
        let flattened_curr_leaf_vec = curr_leaf_vec
            .into_iter()
            .map(|curr_leaf| unbatch_mles(curr_leaf))
            .collect_vec();

        let flattened_curr_dec = unbatch_mles(flattened_curr_dec_vec);
        let flattened_curr_leaf = unbatch_mles(flattened_curr_leaf_vec);

        let two_times_plus_sign_vals = two_times_plus_sign_vals_vec
            .into_iter()
            .map(|two_times_plus_sign_vals| unbatch_mles(two_times_plus_sign_vals))
            .collect_vec();
        ...
        let nonzero_gates_add_decision = decision_add_wiring_from_size(
            1 << (flattened_two_times_plus_sign.num_iterated_vars()
                - num_dataparallel_bits
                - num_tree_bits),
        );
        let nonzero_gates_add_leaf = leaf_add_wiring_from_size(
            1 << (flattened_two_times_plus_sign.num_iterated_vars()
                - num_dataparallel_bits
                - num_tree_bits),
        );

        let res_leaf = layers.add_gate(
            nonzero_gates_add_leaf.clone(),
            flattened_two_times_plus_sign.mle_ref(),
            flattened_curr_leaf.mle_ref(),
            Some(num_dataparallel_bits + num_tree_bits),
            BinaryOperation::Add,
        ); // ID is 5
        let res_dec = layers.add_gate(
            nonzero_gates_add_decision.clone(),
            flattened_curr_dec.mle_ref(),
            flattened_two_times_plus_sign.mle_ref(),
            Some(num_dataparallel_bits + num_tree_bits),
            BinaryOperation::Add,
        ); // ID is 6
...
impl<F: FieldExt> PathCheckCircuitBatchedNoMulMultiTree<F> {
    ...
    pub fn add_subcircuit_layers_to_combined_layers(
        &mut self,
        combined_layers: &mut Layers<F, PoseidonSponge<F>>,
        ...
        let two_times_things_vec =
            combined_layers.add_gkr(BatchedLayer::new(two_times_batched_builder_vec)); // ID 1
        ...
        let two_times_plus_sign_vals_vec =
            combined_layers.add_gkr(BatchedLayer::new(two_times_plus_sign_builders_vec)); // ID 2
        ...
        let curr_decision_vec =
            combined_layers.add_gkr(BatchedLayer::new(curr_node_decision_builders_vec)); // ID is 3
        ...
        let res_dec = combined_layers.add_gate(
            nonzero_gates_add_decision.clone(),
            flattened_curr_dec.mle_ref(),
            flattened_two_times_plus_sign.mle_ref(),
            Some(num_dataparallel_bits + num_tree_bits),
        ...
        let res_leaf = combined_layers.add_gate(
            nonzero_gates_add_leaf.clone(),
            flattened_two_times_plus_sign.mle_ref(),
            flattened_curr_leaf.mle_ref(),
            Some(num_dataparallel_bits + num_tree_bits),
        ...
        combined_output_layers
            .into_iter()
            .chain(new_output_layers.into_iter())
            .collect_vec()
    }
}
