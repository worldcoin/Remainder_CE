...
mod tests {
    ...
    use crate::{
        prover::{helpers::test_circuit, GKRCircuit},
        zkdt::{
            binary_recomp_circuit::{
                circuits::{BinaryRecompCircuit, PartialBitsCheckerCircuit},
                dataparallel_circuits::BinaryRecompCircuitBatched,
                multitree_circuits::BinaryRecompCircuitMultiTree,
            },
            cache_upshot_catboost_inputs_for_testing::generate_mles_batch_catboost_single_tree,
            data_pipeline::dummy_data_generator::{generate_dummy_mles, DummyMles},
            input_data_to_circuit_adapter::{
                convert_zkdt_circuit_data_into_mles, load_upshot_data_single_tree_batch,
    ...
    fn test_partial_bits_checker() {
        ...
        let mut transcript = PoseidonSponge::new("Bin Recomp Circuit Transcript");
        let now = Instant::now();
    ...
    fn test_bin_recomp_circuit_catboost_multitree() {
        ...
        let trees_batched_data: Vec<BatchedZKDTCircuitMles<Fr>> = (0..2)
            .map(|tree_num| {
        ...
        ): (Vec<_>, Vec<_>, Vec<_>) = trees_batched_data
            .into_iter()
            .map(|batched_mle| {
                (
