...
impl<F: FieldExt> GKRCircuit<F> for BinaryRecompCircuitBatched<F> {
    ...
    fn synthesize(&mut self) -> Witness<F, Self::Transcript> {
        ...
        debug_assert_eq!(
            ...
            log2(self.batched_permuted_inputs_mle.len()) as usize
        );
        debug_assert_eq!(
            num_dataparallel_bits,
            log2(self.batched_diff_signed_bin_decomp_mle.len()) as usize
        );
        ...
        let batched_diff_builder = BatchedLayer::new(
            self.batched_decision_node_path_mle
                .iter_mut()
                .zip(self.batched_permuted_inputs_mle.iter_mut())
                .map(|(decision_node_path_mle, permuted_inputs_mle)| {
                    ...
                    decision_node_path_mle.set_prefix_bits(Some(
                        combined_batched_decision_node_path_mle
                            .get_prefix_bits()
                            .iter()
                            .flatten()
                            .cloned()
                    ...
                    permuted_inputs_mle.set_prefix_bits(Some(
                        combined_batched_permuted_inputs_mle
                            .get_prefix_bits()
                            .iter()
                            .flatten()
                            .cloned()
        ...
        let batched_recomp_checker_builder = BatchedLayer::new(
            self.batched_diff_signed_bin_decomp_mle
                .iter_mut()
                .zip(
        ...
        let flattened_batched_recomp_checker_result_mle =
        ...
        Witness {
            layers,
            output_layers: vec![flattened_batched_recomp_checker_result_mle.get_enum()],
...
impl<F: FieldExt> BinaryRecompCircuitBatched<F> {
    ...
    pub fn yield_sub_circuit(
        ...
        let batched_diff_signed_bin_decomp_mle_prefix_bits =
            self.batched_diff_signed_bin_decomp_mle[0].get_prefix_bits();
        // --- First we create the positive binary recomp builder ---
        let pos_bin_recomp_builders = self
            .batched_diff_signed_bin_decomp_mle
            .iter_mut()
