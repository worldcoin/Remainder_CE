...
pub fn load_data_from_serialized_inputs<P: PrimeOrderCurve>(
    image_file_path: &str,
    blinding_factors_file_path: &str,
    commitment_file_path: &str,
    iris_code_file_path: &str,
    iris_code_key_value: &str,
    data_directory: PathBuf,
    im_num_rows: usize,
    im_num_cols: usize,
) -> (Vec<P>, Vec<P::Scalar>, WorldcoinData<P::Scalar>) {
    let image_data = read_bytes_from_file_bin(image_file_path);
    assert_eq!(image_data.len(), im_num_rows * im_num_cols);
    let image: Array2<u8> = Array2::from_shape_vec((im_num_rows, im_num_cols), image_data).unwrap();
    ...
    let vec_iriscode: Vec<bool> = iris_code.clone().into_iter().collect();
    dbg!(vec_iriscode.len());
    let string_iriscode: String = vec_iriscode
        .iter()
    ...
    let string_iriscode_file: String = iris_code_vec_from_file
        .iter()
        .map(|element| (*element as usize).to_string())
        .collect();
    dbg!(string_iriscode_file);

    // get the iriscode from the custody package
    // serde_json::

    let i64image = image.iter().map(|pixel| *pixel as i64).collect_vec();
    let array2image = Array2::from_shape_vec((im_num_rows, im_num_cols), i64image).unwrap();
    ...
    (
        ...
        WorldcoinData {
            ...
            wirings,
            responses,
            iris_code: iris_code_vec_from_file,
            _marker: PhantomData,
        },
    )
}
