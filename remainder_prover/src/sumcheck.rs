            ...
            mles.iter()
                .map(|mle| {
                    ...
                    let mle_index = if mle.num_free_vars() < max_num_vars {
                        ...
                        // then we perform this wrap-around functionality by
                        // first rounding to the nearest power of 2, and then
                        // taking the mod of this index as we implicitly pad for
                        // powers of 2.
...
/// this function performs the same funcionality as the above, except it is when
/// the mle refs we are working with have no independent variable. therefore, we
/// are actually just taking the sum over all of the variables and do not need
/// evaluations.
pub(crate) fn successors_from_mle_product_no_ind_var<F: Field>(
    ...
    let evals_vec = if max_num_vars > 0 {
        let evals = cfg_into_iter!((0..1 << (max_num_vars))).map(|index| {
            // we take the element-wise product at each of the points instead of
