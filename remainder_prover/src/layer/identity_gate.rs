...
impl<E: ExtensionField> Layer<E> for IdentityGate<E> {
    ...
    fn initialize_rlc(&mut self, random_coefficients: &[E], claims: &[&RawClaim<E>]) {
        ...
        let (g2_challenges_vec, g1_challenges_vec): (Vec<&[E]>, Vec<&[E]>) = claims
            .iter()
    ...
    fn bind_round_variable(&mut self, round_index: usize, challenge: E) -> Result<()> {
        if round_index < self.num_dataparallel_vars {
            ...
            [a_hg_mle, &mut self.source_mle].iter_mut().for_each(|mle| {
                mle.fix_variable(round_index, challenge, &mut self.bind_list);
            });
            Ok(())
        }
    }
...
pub struct IdentityGate<E: ExtensionField> {
    ...
    source_mle: DenseMle<E>,
    /// Bind values for `source_mle`
...
impl<E: ExtensionField> IdentityGate<E> {
    ...
    fn append_leaf_mles_to_transcript(
        ...
        assert!(self.source_mle.is_fully_bounded(&self.bind_list));
        transcript_writer
    ...
    /// evaluations.
    fn init_phase_1(&mut self, challenge: &[E], fully_bound_beta_g2: E) {
