...
use crate::{
    claims::RawClaim,
    expression::{generic_expr::Expression, prover_expr::ProverMle},
...
fn regular_layer_test_prove_verify_product() {
    ...
    let mut layer = RegularLayer::new_raw(
    ...
    expression.index_mle_indices_no_bind_list(0);
...
fn regular_layer_test_prove_verify_sum() {
    ...
    let lhs = Expression::<Fr, ProverMle<Fr>>::mle(mle_1);
    let rhs = Expression::<Fr, ProverMle<Fr>>::mle(mle_2);
    let mut expression = Expression::<Fr, ProverMle<Fr>>::sum(lhs, rhs);
...
fn regular_layer_test_prove_verify_complex() {
    ...
    let leaf_mle_1 = Expression::<Fr, ProverMle<Fr>>::mle(mle_1.clone());
    let leaf_mle_2 = Expression::<Fr, ProverMle<Fr>>::mle(mle_2.clone());
    ...
    let mut layer =
        RegularLayer::new_raw(crate::layer::LayerId::Layer(0), root.clone(), Vec::new());
    ...
    root.index_mle_indices_no_bind_list(0);
    let verifier_layer = RegularLayerDescription::new_raw(LayerId::Layer(0), circuit_expression);
