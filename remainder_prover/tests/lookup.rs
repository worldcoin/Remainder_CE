...
pub fn multi_shred_test() {
    let circuit = LayouterCircuit::new(|ctx| {
        ...
        let constrained_2 = get_input_shred_from_vec(
            vec![Fr::from(4u64), Fr::from(3u64), Fr::from(3u64), Fr::from(4u64)],
            ctx);
        let multiplicities_2 = get_input_shred_from_vec(
        ...
        let constrained_3 = get_input_shred_from_vec(
            vec![Fr::from(4u64), Fr::from(4u64), Fr::from(4u64), Fr::from(3u64)],
            ctx);
        let multiplicities_3 = get_input_shred_from_vec(
            vec![Fr::from(1u64), Fr::from(3u64)],
            ctx);
        ...
        let nodes: Vec<NodeEnum<Fr>> = vec![
            ...
            constrained_2.into(), multiplicities_2.into(), lookup_shred_2.into(),
            constrained_3.into(), multiplicities_3.into(), lookup_shred_3.into()
        ];
        ComponentSet::<NodeEnum<Fr>>::new_raw(nodes)
    });
    test_circuit(circuit, None)
...
pub fn test_not_satisfied() {
    let circuit = LayouterCircuit::new(|ctx| {
        ...
        let lookup_node = LookupNode::new(ctx, table.id());
        let constrained = get_input_shred_from_vec(
            vec![Fr::from(3u64), Fr::from(1u64), Fr::from(1u64), Fr::from(1u64)],
            ctx);
        let multiplicities = get_input_shred_from_vec(
            vec![Fr::from(1u64), Fr::from(3u64)],
            ctx);
