...
use remainder::{
    ...
    sumcheck::{compute_sumcheck_message_beta_cascade, SumcheckEvals},
};
...
fn create_dummy_regular_layer(config: BenchLayerConfig) -> (RegularLayer<Fr>, Claim<Fr>) {
    ...
    match config.expression_structure {
        ExpressionStructure::Sum => {
            let leaf_expression = Expression::<Fr, ProverExpr>::mle(leaf_mle);
        ...
        ExpressionStructure::Product => {
            let layer_expression =
                Expression::<Fr, ProverExpr>::products(vec![leaf_mle; config.degree]);
        ...
        ExpressionStructure::BalancedTree => {
            let prod_expression =
                Expression::<Fr, ProverExpr>::products(vec![leaf_mle.clone(); config.degree]);
            let prod_expression_copy = prod_expression.clone();
            let level1_expression =
                Expression::<Fr, ProverExpr>::select(prod_expression, prod_expression_copy);
            let level1_expression_copy = level1_expression.clone();
            let level0_expression =
                Expression::<Fr, ProverExpr>::select(level1_expression, level1_expression_copy);
        ...
        ExpressionStructure::UnbalancedTree => {
            ...
            let mle_ref_n_2 = create_dummy_mle(config.mle_size - 2);

            let mle_ref_n_1 = create_dummy_mle(config.mle_size - 1);
            let mle_n_1_expression = Expression::<Fr, ProverExpr>::mle(mle_ref_n_1);

            let mle_ref_n = create_dummy_mle(config.mle_size);
            let mle_n_expression = Expression::<Fr, ProverExpr>::mle(mle_ref_n);

            let prod_expression =
                Expression::<Fr, ProverExpr>::products(vec![mle_ref_n_2; config.degree]);

            let level1_expression =
                Expression::<Fr, ProverExpr>::select(prod_expression, mle_n_1_expression);
            let level0_expression =
                Expression::<Fr, ProverExpr>::select(level1_expression, mle_n_expression);
