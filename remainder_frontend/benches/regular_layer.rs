...
fn create_dummy_regular_layer(config: BenchLayerConfig) -> (RegularLayer<Fr>, RawClaim<Fr>) {
    ...
    match config.expression_structure {
        ...
        ExpressionStructure::UnbalancedTree => {
            ...
            let mle_n_1_expression = Expression::<Fr, ProverExpr>::mle(mle_n_1);

            let mle_n = create_dummy_mle(config.mle_size);
            let mle_n_expression = Expression::<Fr, ProverExpr>::mle(mle_n);

            let prod_expression =
                Expression::<Fr, ProverExpr>::products(vec![mle_n_2; config.degree]);
