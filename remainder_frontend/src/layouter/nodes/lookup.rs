...
impl LookupConstraint {
    ...
    /// The adding of lookup specific verifier-challenge- and output layers is handled automatically
    /// by [LookupTable::generate_lookup_circuit_description].
    ///
    /// # Requires:
...
pub struct LookupTable {
    ...
    /// [LookupTable::add_lookup_constraint], during layout).
...
>>>>>>> benny/extract_frontend:remainder_frontend/src/layouter/nodes/lookup.rs
        ...
        let (fiat_shamir_challenge_location, fiat_shamir_challenge_node_vars) =
            circuit_map.get_location_num_vars_from_node_id(&self.fiat_shamir_challenge_node_id)?;
        ...
        let constrained_expr = AE::<F>::binary_tree_selector(
            self.constraints
                .iter()
        ...
        let expr = CE::sum(
            CE::mle(fiat_shamir_challenge_mle),
            CE::negated(constrained_expr.build_circuit_expr(circuit_map)?),
        );
        ...
        let (multiplicities_location, multiplicities_num_vars) = circuit_map
            .get_location_num_vars_from_node_id(&self.constraints[0].multiplicities_node_id)
            .unwrap();
        ...
        if self.constraints.len() > 1 {
            ...
            let expr = self.constraints.iter().skip(1).fold(
                CE::mle(rhs_numerator_desc),
                |acc, constraint| {
                    let (multiplicities_location, multiplicities_num_vars) = &circuit_map
                        .get_location_num_vars_from_node_id(&constraint.multiplicities_node_id)
                        .unwrap();
                    ...
                    acc + CE::mle(mult_constraint_mle_desc)
                },
            );
            let layer_id = LayerId::next_layer_id();
            let layer = RegularLayerDescription::new_raw(layer_id, expr);
            ...
            let (_first_self_constraint_loc, first_self_constraint_num_vars) = circuit_map
                .get_location_num_vars_from_node_id(&self.constraints[0].multiplicities_node_id)
                .unwrap()
                .clone();
        ...
        let (fiat_shamir_challenge_loc, fiat_shamir_challenge_num_vars) = circuit_map
        ...
        let (table_loc, table_num_vars) = circuit_map
            .get_location_num_vars_from_node_id(&self.table_node_id)
            .unwrap()
            .clone();
        ...
        let expr = CE::mle(fiat_shamir_challenge_circuit_mle) - CE::mle(table_circuit_mle);
...
fn build_fractional_sum<F: Field>(
    ...
    for i in 0..denominator_desc.num_free_vars() {
        ...
        let next_numerator_expr = if let Some(numerator_desc) = maybe_numerator_desc {
            ...
            CE::mle(denominators.1.clone()) + CE::mle(denominators.0.clone())
        };
        ...
        println!("Iteration {i} of build_fractional_sumcheck has layer id: {layer_id:?}");
